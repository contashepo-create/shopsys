...
- **TR-2 ربط المستخدم والمنتقي (2026-09-22):** `AppUser.treasuryAccess` يحفظ الخزائن المتعددة والافتراضي، و`TreasuryPicker` يقبل operation اختيارية ليعرض المسموح للمستخدم الحالي فقط ورسالة واضحة عند انعدام الحسابات. الإخفاء UX وليس حارس الأمان؛ Repository يفرض السياسة في TR-3.
