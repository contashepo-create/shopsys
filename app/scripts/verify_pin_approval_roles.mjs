...
useDataStore.setState({ ownerPinHash: await hashPin('999999') })
...
st().addAppUser({ nameAr: 'كاشير أحمد', roleId: 'cashier', pinHash: await hashPin('111111'), employeeId: e1.id })
st().addAppUser({ nameAr: 'مشرف سامي', roleId: 'branch_manager', pinHash: await hashPin('222222'), employeeId: e2.id })
st().addAppUser({ nameAr: 'محاسب منى', roleId: 'accountant', pinHash: await hashPin('333333'), employeeId: e3.id })
st().addAppUser({ nameAr: 'كاشير موقوف', roleId: 'branch_manager', pinHash: await hashPin('444444'), employeeId: e4.id })
...
ok('رقم المالك يعتمد مرتجعاً', (await tryApprove('999999', 'sales.return.approve')) === 'المالك')
ok('رقم المالك يعتمد صرف نقدية', (await tryApprove('999999', 'trs.payment.approve')) === 'المالك')
// ② كاشير بلا صلاحية الاعتماد يُرفض رقمه الصحيح
const r2 = await tryApprove('111111', 'sales.return.approve')
ok('رقم الكاشير الصحيح يُرفض لاعتماد مرتجع (لا صلاحية) — ' + r2.slice(0, 40), r2.startsWith('ERR:'))
// ③ المشرف (manager) يعتمد المرتجع
const r3 = await tryApprove('222222', 'sales.return.approve')
ok('رقم المشرف يعتمد المرتجع باسمه: ' + r3, r3 === 'مشرف سامي')
// ④ المحاسب يعتمد صرف النقدية (صلاحية خزينة)
const r4 = await tryApprove('333333', 'trs.payment.approve')
ok('المحاسب وصرف النقدية: ' + r4, r4 === 'محاسب منى' || r4.startsWith('ERR:'))
// ⑤ مستخدم موقوف رقمه لا يعتمد شيئاً حتى لو دوره مشرف
const r5 = await tryApprove('444444', 'sales.return.approve')
...
const r7 = await tryApprove('999999', 'sales.return.approve')
