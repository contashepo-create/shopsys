...
- **INV-5.1 مرتجع الشراء بالسطر (2026-09-22):** `PurchaseReturnLine` يحمل `purchaseLineIndex` و`warehouseId`، ومحرك `buildPurchaseReturnLinesPerLine` يحفظ تكلفة/سعر/مخزن السطر بلا متوسط يخلط سطرين. السجلات القديمة المجمعة تُستهلك FIFO توافقياً. الدمج في Repository والواجهة بالدفعة التالية.
