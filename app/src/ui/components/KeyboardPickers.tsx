  ...
  return <div ref={pickerRef} className={`relative ${open ? 'z-[70]' : ''}`} data-enter-native="true">
    ...
    {open && <div className="fixed inset-0 z-[60] bg-slate-950/35 p-4 sm:p-8" onMouseDown={() => setOpen(false)}>
      <div role="dialog" aria-label={`${label} — نتائج البحث`} className="mx-auto mt-[8vh] flex max-h-[78vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-brand-300/50 bg-white shadow-2xl dark:border-brand-700/50 dark:bg-card-dark" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="overflow-auto p-2">{showCash && <button type="button" data-quick-option="true" data-value={String(emptyValue)} onMouseDown={(event) => event.preventDefault()} onClick={() => { onChange(emptyValue); setOpen(false); onConfirm?.() }} className="w-full rounded-xl border border-dashed border-brand-300 p-3 text-right font-bold text-brand-700 hover:bg-brand-500/10 dark:border-brand-700 dark:text-brand-300">{cashLabel}</button>}{matches.length ? matches.map(renderParty) : <div className="p-8 text-center text-sm text-slate-400">لا توجد نتائج مطابقة</div>}</div>
      </div>
    </div>}
  </div>
