...
- **⑦ سياسة الوردية بحسب الدور والسياق** (نمط Toast/Square): النواة المشتركة `salesShiftPolicy()` في `core/shifts.ts` تُستخدم في `repo.ts` و`PosPage.tsx`. أنشطة «الفاتورة أولاً» مستثناة. الحساب المفتوح للمالك الرئيسي يحصل على تلميح فقط ولا يُمنع، والكاشير يتبع `setup.requireOpenShiftForSales`، أما المدير/المحاسب/أي دور موظف آخر في سياق البيع فيُلزم بورديّة مفتوحة حتى لو عُطّل مفتاح الكاشير. رسالة الرفض عربية ومحددة بالسياق؛ شارة POS والهيدر يوجهان لشاشة الورديات، ولا يختار F8/F9 زرًا خلفيًا أو صفاً عشوائياً. صلاحية شاشة الورديات مضافة للأدوار الموظفة التي قد تنفذ سياقاً يحتاج وردية. فحص السياسة: `verify_shift_role_policy.mjs`، وفحص التشغيل العام: `verify_owner_batch8_points.mjs` (المالك اختياري + الربط عند فتح الوردية).
