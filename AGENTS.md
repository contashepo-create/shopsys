...
**«تَحَكَّم» TAHAKAM ERP** — نظام محاسبة وكاشير وإدارة عربي متعدد الأنشطة (29 نشاطاً) متعدد الدول (19 دولة عربية)، منافس لـEasy Store وأنظمة الأنشطة الرأسية العالمية (Foodics للمطاعم، RepairDesk للموبايلات، أنظمة الصاغة، pro-acc للمقاولات…). الهدف: **محاسبة متكاملة وإدارة متكاملة لكل نشاط** بمحرك قيود مزدوجة مخفي التعقيد عن المستخدم.
...
└── app/                         ← تطبيق Vite + React 19 + TypeScript + Tailwind v4 + zustand v5
    ├── scripts/                 ← 159 سكربت تحقق verify_*.mjs + license_tool.mjs
...
- البيانات في localStorage مشفرة عبر `secureStorage.ts`، `DATA_VERSION = 25` مع ترحيل بافتراضات `?? []` لكل مصفوفة جديدة؛ الإصدار 25 يستعيد سندات الصرف القديمة لسداد استحقاقات مصروفات الشراء.
...
# الحزمة الكاملة (159 سكربتاً — يجب 0 فشل):
for f in scripts/verify_*.mjs; do node --experimental-strip-types "$f" >/dev/null 2>&1 || echo "FAILED: $f"; done
```

**الوضع الحالي (2026-09-26): 159/159 سكربت تحقق ناجحاً، و70 ملف Vitest تضم 293 اختباراً ناجحاً، وصفر فشل.** أي ميزة جديدة = سكربت تحقق جديد يغطي القيود والرفض والتوازن.
...
- **الاستحقاق ومركز تكلفة الأسطول (الإصدار 25)**: مصروف الشراء يمكن إثباته `paidBy: 'payable'` مع `beneficiaryName` و`payableAccountCode` بلا دفع فوري. ينشئ ذلك قيد تكلفة/استحقاق وسجل `PurchaseExpensePayable` مستقل؛ السداد لاحقاً عبر `settlePurchaseExpensePayable` يقيد 2117/الخزينة ولا يكرر التكلفة. عند السداد النقدي الفعلي ينشأ `Voucher` من نوع `payment` برقم `PV-####` ويرتبط بـ`purchaseExpensePayableId`، ويُضاف القيد إلى اليومية والسندات. مسار الترحيل في DATA_VERSION 25 يعيد ربط السندات القديمة من `settlementEntryIds` دون تكرار. هذا المسار متاح للمصروف اللاحق في `PurchasesPage` و`VouchersPage` وللمحرر المتقدم.
...
- **اختصار F9 في الشراء**: يعتمد `PurchaseInvoicePage` فعلياً عبر callback واحد؛ زر «ترحيل F9» و`data-shortcut="F9"` مجرد واجهتين للعملية نفسها، وتمنع `KeyboardNavigation` التكرار. بعد نجاح الترحيل ينتقل المستخدم إلى `/purchases/invoices`. التحقق المستهدف 12/12، والحزمة الكاملة 159/159.
...
- `npx vitest run` (أو `npm run test:e2e`) — ملفان: `e2e_registration.test.tsx` (7 اختبارات: تسجيل كامل من المعالج لكل عائلة نشاط بالنقر الفعلي، عزل الأقسام الصارم في Sidebar، لا تلوث بين المستأجرين، سلامة القوالب الـ29) و`e2e_cross_screen.test.tsx` (4: المنسدلات تجلب أسماء/مفاتيح حية — عملاء في المشاريع، موظفون وأصناف برصيد حي في أذون الصرف مع تنفيذ كامل، فواتير مفتوحة حقيقية في التحصيلات، موردون في عقود الباطن).
...
### SALE-INVOICE-45 — تطبيق فئة خصم العميل وتحسين محرر فاتورة المبيعات (2026-09-25)
- أصلح محرر فاتورة المبيعات المتقدمة بحيث يضيف الصنف بسعر قائمة العميل الفعلي، ويعيد تسعير السطور عند تغيير العميل؛ لم يعد يعتمد دائماً على سعر التجزئة.
- أضيفت بطاقة بيانات عميل منظمة أعلى الفاتورة تعرض الاسم، الهاتف، البريد، الرقم الضريبي، السجل التجاري، العنوان، الهوية، الملاحظات، فئة الخصم وحد الائتمان.
- أعيد ترتيب الواجهة بصرياً: رأس فاتورة واضح، بطاقة بيانات فاتورة، جدول أصناف ببحث داخل شريط منظم ورسالة حالة فارغة، وبطاقتا تحصيل وإجمالي مستقلتان؛ النصوص والحقول أصبحت داخل حدود البطاقات ولا تهبط خارجها، ويظل ذلك مناسباً للوضع المبسط.
- الفحص المستهدف: `npm run typecheck`، اختبارات الفواتير/قوائم الأسعار/منتقيات لوحة المفاتيح، و`git diff --check`.

### PRICELIST-INLINE-44 — تعديل أسعار الأصناف مباشرة من الجدول (2026-09-25)
- أضيف زر «بدء التعديل» إلى جدول أسعار الأصناف؛ يتحول إلى «إنهاء التعديل» ويعرض حقولاً مباشرة داخل خلايا السعر القطاعي وأسعار فئات الخصم النشطة.
- الحفظ سريع بالضغط على Enter أو مغادرة الخلية، وإفراغ سعر فئة الخصم يعيد تطبيق الخصم الافتراضي. Escape يعيد قيمة الخلية السابقة دون تغيير.
- أضيف اختبار واجهة يثبت فتح وضع التعديل وحفظ السعر القطاعي وسعر الفئة مباشرة. الفحص: `npm run typecheck`، build، `npx vitest run` (67 ملفاً/269 اختباراً)، `npm run lint:src` بصفر أخطاء، و`git diff --check`.

### INV-QUICKPICKERS-46 — تعميم منتقيات الفواتير والضريبة (2026-09-25)
- عُمّم `ItemQuickPicker` و`PartyQuickPicker` على فاتورة الشراء البسيطة وتعديل فواتير البيع والشراء؛ لم تعد اختيارات الصنف أو العميل أو المورد أو الموظف في هذه المسارات تعتمد على `<select>`. البحث يبدأ فوراً من أول حرف، والاختيار بـ Enter، مع دعم F4 وF7 لفتح بحث الطرف وF5 لفتح بحث الصنف.
- أبقي العميل/المورد النقدي قيمة افتراضية واضحة، وأضاف منتقي الطرف دعماً للقيم النقدية المخصصة وإخفاء خيار النقد عند اختيار الموظف؛ بعد تأكيد الطرف ينتقل التركيز إلى بحث الصنف، كما أصبح F5 يركز فعلياً على أول منتقٍ في الشراء البسيط.
- أصبحت بطاقتا التحصيل والإجمالي في فاتورة البيع المتقدمة متجاورتين ومتساويتين على الشاشات الواسعة، وأضيف زر «المصاريف» بجوار الإجمالي لفتح إدارة المصروفات.
- طبقت فواتير المبيعات/المشتريات المتقدمة والبسيطة والكاشير سياسة `resolveBusinessTax` عند الضريبة المعفاة/الصفرية: النسبة، العرض، ضريبة المدخلات، إضافات العميل والمصاريف والترحيل تصبح صفرية/غير خاضعة دون إعادة كتابة مسارات الترحيل.
- الفحص: `npm run typecheck`، `npm run build`، `npm run test:e2e`، `npm run lint:src` (تحذيرات المشروع القائمة مقبولة)، و`git diff --check`.

### INV-PURCHASE-43 — حماية حذف الأصناف ومنتقي الموردين والمواد (2026-09-25)
- حذف الصنف لم يعد فورياً: تأكيد مستقل يوضح الحذف النهائي ثم يطلب الرقم السري؛ لا يظهر الزر كمسموح إلا للمالك أو لفئة موظفين تحمل صلاحية `inv.item.delete`، ويتحقق Repository من الصلاحية والاعتماد قبل الحذف ويسجل المعتمد.
- أضيفت صلاحية حذف مستقلة إلى مصفوفة CRUD للأصناف، فلا تكفي صلاحية التسوية المخزنية، ويمكن للمالك منحها لفئات الموظفين المخصصة من شاشة الصلاحيات.
- استُبدل مورد فاتورة الشراء التقليدية بمنتقي كتابة منبثق: الكتابة تعرض الموردين، Enter يختار ويضع التركيز على حقل الصنف، والشراء النقدي بقي خياراً مستقلاً. حقل الصنف الأول يستقبل التركيز فعلياً بعد التأكيد.
- أزيل حد أول 30 نتيجة من منتقي الموردين والأصناف حتى تظهر كل المواد المسجلة مع استمرار البحث والتمرير داخل القائمة.
- الفحص: `npm run typecheck`، build، اختبارات لوحة المفاتيح والصلاحيات، و`git diff --check`؛ lint يمر بصفر أخطاء مع تحذيرات المشروع المعروفة.

### PRICELIST-CATALOG-42 — جدول أسعار الأصناف وفئات الخصم (2026-09-25)
- أضيف زر «جدول أسعار الأصناف» إلى شاشة قوائم الأسعار؛ يفتح جدولاً موحداً بكل الأصناف مع السعر القطاعي، وسعر كل فئة نشطة، ونسبة الخصم الفعلية.
- أضيفت إمكانية تفعيل/إضافة فئة «جملة» سريعاً، مع بقاء أي فئة أخرى قابلة للإضافة والتعديل؛ الضغط على عنوان الفئة في الجدول يفتح محرر أسعارها الخاصة.
- أضيف اختيار «فئة الخصم / قائمة الأسعار» إلى نموذج العميل، وتظهر الفئة أيضاً في بطاقات العملاء وجدولهم؛ العميل المربوط بها يستخدمها في تسعير الكاشير الموجود مسبقاً.
- زر حفظ الصنف يحمل F9 صراحة، وسندات القبض/الصرف تحتفظ بزر «حفظ السند» المربوط بـF9؛ لا يتغير F8 الخاص بالمسودة.
- الفحص: `npm run typecheck`، اختبارات لوحة المفاتيح — 16 اختباراً ناجحاً، و`git diff --check`.

### DESKTOP-SQLITE-41 — أول مرآة منظمة لدفتر اليومية في SQLite (2026-09-25)
- رُفع مخطط SQLite إلى الإصدار 2 مع `journal_entries` و`journal_lines` وفهارس التاريخ والحساب ومفتاح أجنبي مركب يحافظ على سطور القيد.
- تستخرج طبقة التخزين القيود الجديدة من لقطة Zustand، وتثبت القيد وسطوره داخل نفس معاملة اللقطة والتدقيق؛ القيد append-only، وتعارض نفس المعرف بمحتوى مختلف يُرفض عبر بصمة SHA-256.
- الترحيل الأولي ينسخ اليومية الموجودة إلى الجداول المنظمة مع إبقاء `journal` في Zustand للتوافق. لا تزال هذه مرآة انتقالية وليست استبدالاً لمصدر القراءة الحالي أو Repository المحاسبي الكامل.
- أضيف اختبار استخراج قيد جديد مع `costCenterId` وتحديث عقد/اختبارات المخطط إلى الإصدار 2.
- الفحص المستهدف: `npm run typecheck`، اختبارات التخزين والمخطط — 8 اختبارات، و`node --check` لملفات Electron.

### DESKTOP-SQLITE-40 — عكس سجل التدقيق إلى SQLite بمعاملة اللقطة (2026-09-25)
- تستخرج طبقة التخزين أحداث التدقيق الجديدة من payload Zustand فقط، وتمررها مع حفظ اللقطة؛ `main.cjs` يثبتها في `audit_events` داخل نفس معاملة `store_state` وبمعرف فريد لكل مخزن/حدث.
- الترحيل الأولي ينسخ السجل الموجود إلى الجدول بعد نجاح حفظ SQLite، مع استمرار `auditLog` في اللقطة للتوافق والواجهة. لا يُعد هذا نقل المصدر التجاري الكامل؛ هو مرآة تدقيق محلية قابلة للتوسع.
- أضيف اختبار استخراج الأحداث الجديدة، مع الحفاظ على تعقيم/حدود الحدث في طبقة Electron.
- الفحص المستهدف: `npm run typecheck`، اختبار التخزين — 5 اختبارات، و`node --check` لملفات Electron.

### DESKTOP-SQLITE-39 — عقد outbox محلي قابل لإعادة المحاولة (2026-09-25)
- أضيف إلى جسر SQLite مسار `enqueueOutbox` للإدراج الفريد، و`claimOutbox` لمطالبة الأحداث الجاهزة ذرياً مع زيادة `attempts`، و`completeOutbox` لوضع `sent/failed` وموعد المحاولة التالية.
- التحقق مركزي في Electron: JSON صالح، حدود حجم وحقول الحدث، حالات انتقال محددة، وقفل المعاملة يمنع نافذتين من مطالبة الحدث نفسه.
- رُبطت عمليات IPC بالـpreload، بينما بقيت أحداث الأعمال غير مربوطة بهذا المسار عمداً حتى يتم نقل ترحيل المستندات إلى Repository ذري؛ هذه مرحلة تأسيس outbox وليست ادعاء مزامنة مكتملة.
- الفحص المستهدف: `npm run typecheck`، اختبارات التخزين/مخطط SQLite — 6 اختبارات، و`node --check` لملفات Electron.

### DESKTOP-SQLITE-38 — منع تكرار حفظ لقطات SQLite بعد انقطاع IPC (2026-09-25)
- يولد `persistentStorage.ts` مفتاحاً ثابتاً من اسم المخزن والإصدار ومحتوى اللقطة عبر SHA-256، ويمرره إلى SQLite مع كل حفظ أو ترحيل أولي.
- يحفظ `main.cjs` نتيجة العملية ومفتاحها داخل `idempotency_keys` في نفس معاملة تحديث `store_state`؛ إعادة إرسال الطلب نفسه تعيد النتيجة بلا كتابة ثانية، أما إعادة استعمال المفتاح لمحتوى مختلف فمرفوضة.
- أضيف فهرس زمني لمفاتيح منع التكرار، ويحتفظ مسار اللقطات بآخر 256 مفتاحاً فقط حتى لا تنمو قاعدة البيانات بلا حد. لا تزال هذه حماية لقطة التخزين وليست بديلاً عن ربط ترحيل المستندات الذري.
- الفحص المستهدف: `npm run typecheck`، اختبارات التخزين/مخطط SQLite — 6 اختبارات، و`node --check` لملفات Electron.

### DESKTOP-SQLITE-37 — وصل Zustand بلقطة SQLite في نسخة Electron (2026-09-25)
- أضيف `persistentStorage.ts` كحد تخزين واحد: الويب يستمر على `secureStorage`، وElectron يمرر `shopsys-data` إلى SQLite عبر IPC دون تغيير واجهات Zustand.
- الحفظ والقراءة والحذف تعمل بطابور لكل مخزن مع `expectedRevision`؛ تعارض نافذة أخرى يُرفض ولا يُكتب فوق لقطة أحدث بصمت.
- payload اللقطة يُشفّر في `desktop/main.cjs` عبر `safeStorage` عند توفر keyring، مع دعم صريح للـfallback وقراءة اللقطات القديمة غير المشفرة؛ أضيف حذف اللقطة إلى عقد IPC.
- عند أول تشغيل Electron مع تخزين ويب قديم، تُحفظ نسخة SQLite أولاً ثم تُحذف النسخة القديمة، ويعاد المحاولة إذا فشل الحفظ. نقل جداول الأعمال من اللقطة إلى جداول SQLite التفصيلية ما زال مرحلة لاحقة.
- أضيفت 3 اختبارات لجسر التخزين (قراءة/كتابة/حذف، ترحيل قديم، وتعارض)، وتحديث `desktop/README.md`.
- الفحص: `npm run typecheck`، اختبارات SQLite/التخزين/المحاكي — 8 اختبارات ناجحة، و`node --check` لملفات Electron.

### COST-CENTER-GENERAL-5 — تحميل تكاليف المقاولات على المركز العام (2026-09-25)
- أضيف `costCenterId` اختيارياً إلى `ProjectCost` مع إبقاء `projectId` كربط مشروع مستقل، وإضافة الاختيار إلى نموذج تكلفة المشروع.
- سطر 5110 المدين في قيد تكلفة المشروع يحمل المركز العام عند اختياره، بينما تبقى الضريبة وحساب السداد بلا وسم تلقائي؛ يتحقق Repository من أن المركز نشط.
- أضيف اختبار ترحيل تكلفة مشروع آجلة يثبت وجود `projectId` و`costCenterId` معاً وسلامة قيد 5110.
- الفحص: `npm run typecheck` والاختبارات المستهدفة — 8 اختبارات في `cost_centers_general.test.ts` ناجحة.

### COST-CENTER-GENERAL-4 — تتبع ارتباط المرتجعات بالمراكز (2026-09-25)
- أضيف `returnsByCostCenter` لتجميع مرتجعات البيع والشراء المرتبطة بالمراكز الموروثة من الفاتورة، مع إبقائها تقرير ارتباط مستقلاً لا يخلط قيمة المرتجع بإجمالي المصروفات أو مركز تكلفة المركبة.
- أضيفت بطاقة في تقارير المصروفات تعرض نوع المرتجع وعدده وقيمته لكل مركز، مع الحفاظ على `costCenterIds` في مستندات المرتجع ومنع حذف المركز المرتبط.
- أضيف اختبار يغطي المرتجع المرتبط بمركز واحد ومرتجع شراء مرتبط بأكثر من مركز.
- الفحص: `npm run typecheck` والاختبار المستهدف `tests/cost_centers_general.test.ts`.

### COST-CENTER-GENERAL-3 — شجرة المراكز والتوزيع والموازنات (2026-09-25)
- توسع نموذج المركز العام بـ`parentId` مع منع الأب الذاتي والدورات، وتمنع طبقة البيانات حذف مركز له أبناء أو مستندات أو موازنات؛ يظل التعطيل هو البديل الآمن.
- أضيفت موازنات مركزية (`costCenterBudgets`) بفترة وقيمة وملاحظات، مع ترحيل persist إلى الإصدار 24، وواجهة إدارة مستقلة داخل شاشة مراكز التكلفة.
- أضيف `allocateJournalLine` لتوزيع سطر قيد على عدة مراكز بأوزان وبخوارزمية الباقي الأكبر، مع حفظ `costCenterId` على السطور الناتجة ومطابقة مجموع السطر حرفياً.
- أضيف تقرير مقارنة الموازنة بالفعلي والانحراف ونسبة الاستخدام، ويظهر في تقارير المصروفات اعتماداً على سطور القيود الموسومة، ويضم المركز الأب مصروفات الأبناء في المقارنة.
- أضيف اختيار المركز العام إلى سطر القيد اليدوي نفسه، مع تحقق أن أي `costCenterId` موجود ونشط قبل الحفظ، فلا تدخل موازنة أو تقرير إحالة إلى مركز محذوف/معطل.
- الفحص: `npm run typecheck`، الاختبار المستهدف `tests/cost_centers_general.test.ts` — 6 اختبارات ناجحة.

### COST-CENTER-GENERAL-2 — وسم سطور القيود وتجميع الأنشطة حسب المركز (2026-09-25)
- أضيف `costCenterId` الاختياري إلى `JournalLine` كسمة للمصروف العام فقط، مع إبقاء `vehicleId`/حركات مركز تكلفة المركبة مساراً مستقلاً لا يُستبدل به.
- قيود المصروف الداخلي في فواتير البيع والشراء، وقيود سندات الصرف ذات الحساب المصروف، تحمل المركز على سطر المدين الفعلي؛ لا تُوسم سطور الضريبة أو الطرف الدائن تلقائياً حتى لا تنسب الالتزام المالي للمركز خطأً.
- امتد تمرير المركز إلى مصروفات فترة الشراء عند إنشاء القيد وعند تعديل الفاتورة، مع استمرار التحقق من نشاط المركز، ومنع ربطه بالقبض أو التحويل أو حساب غير مصروف في السند.
- أضيف `journalExpensesByCostCenter` لتجميع القيود الموسومة حسب المركز والحساب والتاريخ، وأضيفت بطاقة في تقارير المصروفات تعرض القيود المرحّلة من الأنشطة خارج تقرير مستندات الفواتير، مع اختبار يثبت الوسم والتجميع.
- الفحص: `npm run typecheck`، الاختبار المستهدف `tests/cost_centers_general.test.ts` — 5 اختبارات ناجحة.
...
### QUICK-SELECT-LEDGER-2026-09-25 — منتقيات البحث ومعالجة مصروفات الشراء
- استُبدل استعمال عناصر HTML الأصلية `<select>` في `app/src` بالكامل بمكوّن `QuickSelect` المشترك في `ui/components/KeyboardPickers.tsx`. المنتقي يعرض popover قابلاً للكتابة والبحث من أول حرف، والأسهم، وEnter، وEscape، ويحافظ على `onChange` القديمة عبر `event.target.value`. يشمل ذلك صفحات الأنشطة والإعداد الأولي؛ ولا توجد نتيجة `<select` في `app/src` بعد المرحلة.
- بقيت منتقيات الصنف والطرف المتخصصة (`ItemQuickPicker` و`PartyQuickPicker`) هي المسار المفضل للأصناف والعملاء والموردين والموظفين، بينما تعمل القوائم الأخرى بنفس سلوك البحث المنبثق. عناصر الدفع/التحصيل لا تُفقد وظيفتها؛ `QuickSelect` يستقبلها أيضاً بدلاً من قائمة المتصفح.
- عولج مسار فاتورة الشراء المتقدمة: الحساب يفصل تكلفة المصروف، ضريبة المدخلات القابلة للاسترداد، والمبالغ المدفوعة مباشرة من خزينة/بنك أو عهدة أو حساب استحقاق؛ ويعرض مستحق المورد بعد تلك المصادر، مع منع دفع أكبر من المستحق. كما تم تصحيح مبالغ مصروفات الفترة/العهدة والاستحقاقات عند وجود ضريبة، وتحسين fallback لتوزيع السداد في `buildPurchaseEntryV2`.
- أضيف اختبار تكاملي إلى `tests/purchase_expense_tax_repository.test.ts` يرحّل مصروفاً خاضعاً للضريبة مع مصادر `treasury` و`payable` و`custody`، ومصروف فترة، ويتحقق من توازن القيد، تكلفة المخزون، ضريبة المدخلات، ومستحق المورد (6 اختبارات).
- بعد اعتماد الفاتورة المتقدمة أو الرجوع منها يكون المسار `/purchases/invoices`.
- الفحوص النهائية: `npm run typecheck`، `npm run build`، `npm run lint:src` (0 أخطاء، تحذيرات قائمة سابقاً)، و`npm run test:e2e` (67 ملفاً و274 اختباراً ناجحاً)، إضافة إلى `git diff --check`. حلقة `scripts/verify_*.mjs` القديمة ما زالت تحتوي توقعات 28 نشاطاً بينما القوالب الحالية 29، لذلك تفشل فحوصها الثابتة القديمة خارج نطاق هذه المرحلة.

### INV-QUICKPICKERS-47 + EXPENSE-ACCRUAL-48 — إغلاق تعميم المنتقيات وتصحيح مسارات المصروفات (2026-09-25)
- استُبدلت اختيارات الأطراف/الموظفين المتبقية في الشيكات والعمولات الخارجية والأسطول بـ`PartyQuickPicker`؛ يبدأ البحث بالكتابة، ويدعم الأسهم وEnter وEscape، مع إبقاء حالات «بلا طرف/بلا سائق» حيث يلزم.
- عُممت سياسة البحث المنبثق على شاشات الأصناف والأطراف والعمليات ذات الصلة، مع بقاء `QuickSelect` مخصصاً للقيم التشغيلية التي تحتاج قائمة قابلة للبحث مثل المخازن والوحدات والخزائن وطرق الدفع والحسابات ومراكز التكلفة؛ لا توجد عناصر HTML أصلية `<select` داخل `app/src`.
- أضيف `PurchaseExpenseManager` لفاتورة الشراء التقليدية: يختار قالب مصروف نشطاً، يملأ الاسم والحساب ومعالجة التحميل تلقائياً، ويفتح إنشاء قالب جديد من داخل الفاتورة. يدعم الفصل الواضح بين تكلفة المخزون ومصروف الفترة، ومعالجة الضريبة، ومركز التكلفة العام، وربط مركبة الأسطول، ومصدر السداد.
- مصروف «مستحق لجهة أخرى» يتطلب `beneficiaryName` ويُمنع مركزياً من حساب الموردين `2101`. لا يُنشأ مورد نقدي وهمي، ولا يُضاف المصروف المستحق لجهة مستقلة إلى رصيد المورد؛ أما المصروف المستحق للمورد فلا يزال يدخل `supplierDue` صراحةً.
- مصروف نقل المركبة المرتبط بـ`vehicleId` ينشئ حركة `vehicleCostEntries` مستقلة بحالة `accrued` عند عدم الدفع، ويحافظ على أثر تكلفة المركبة/النقلة دون حركة إيراد عامة أو دين مورد وهمي. أضيف اختبار تكاملي يثبت توازن القيد، وحساب الاستحقاق المستقل، وعدم استخدام 2101.
- أضيفت تغطية لمسار مصروف البيع الداخلي المستحق، بما في ذلك اسم الجهة وحساب الاستحقاق وتوازن القيد ومنع حساب الموردين.
- الفحوص النهائية لهذه المرحلة: `npm run typecheck` ناجح؛ `npm run build` ناجح (تحذير حجم bundle من Vite غير حاجز)؛ `npm run lint:src` بصفر أخطاء مع تحذيرات React/Compiler القائمة؛ `npm run test:e2e` نجح بـ68 ملفاً و277 اختباراً؛ و`git diff --check` ناجح قبل الإنهاء الإداري.

### VOUCHER-INVOICE-ALLOCATIONS-49 — تخصيص سندات القبض والصرف وعكسها (2026-09-25)
- أضيف `validatePaymentAllocations` في `core/projectOps.ts` كحارس مستقل: لا يقبل إلا مستنداً مفتوحاً يخص الطرف، يمنع التكرار والقيم غير الصحيحة والتجاوز عن المتبقي أو إجمالي الدفعة، ويعيد الفائض تحت الحساب.
- `postVoucher` يدعم `allocations` اختيارية: عند غيابها يوزع سند قبض العميل أو سند صرف المورد FIFO، ويحفظ `allocations` و`unallocatedMinor` داخل `Voucher`. الحسابان 1104 و2101 يتطلبان طرفاً مسجلاً مركزياً ولا يقبلان طرفاً وهمياً.
- أضيف `getOpenSupplierInvoices` مع احتساب الدفعات السابقة وتوزيعات السندات ومرتجعات الشراء على الحساب؛ وامتد `getOpenClientInvoices` لتجاهل السند المعكوس واحتساب توزيعاته. تظهر الفواتير المفتوحة والتوزيع اليدوي في `VouchersPage` بمنتقيات الأطراف الموحدة، بينما يبقى FIFO هو الافتراضي.
- التخصيصات تدخل موانع تعديل فاتورة البيع وحساب الرد في معالج المرتجعات، ولا تتكرر في كشف الحساب؛ عند عكس السند من `reverseVoucher` أو من اليومية يُحفظ `reversalEntryId` ويُستبعد أثره من كشوف الأطراف وقوائم الفواتير المفتوحة مع إبقاء القيد الأصلي والقيد العاكس Append-only.
- عكس السند يتطلب سبباً وموافقة مشرف، ويُمنع للسندات المرتبطة بماكينة دفع أو مركز تكلفة مركبة أو دفتر مساعد آخر؛ لا تُحذف حركة محاسبية ولا يُسمح بالعكس المزدوج.
- أضيف `tests/voucher_settlement_allocations.test.ts` لاختبار FIFO للعميل والمورد، التخصيص اليدوي وحدوده، الفائض، اشتراط الطرف، وعكس السند وإعادة فتح الفاتورة.
- فحوص الإنهاء: `npm run typecheck` و`npm run build` ناجحان، `npm run lint:src` بصفر أخطاء و36 تحذيراً قائماً، و`npm run test:e2e` يمر بـ69 ملفاً و282 اختباراً. فحوص `scripts/verify_*.mjs` محدثة الآن لتطابق الشجرة الحالية؛ الحلقة الكاملة تمر بـ159/159، وفحوص SQLite/IPC والمقاولات المتأثرة بإلزام الطرف ناجحة.

### RELEASE-CONSISTENCY-2026-09-25 — إغلاق فحوص الأنشطة والمخزون والطباعة
- [قرار تاريخي استُبدل في 2026-09-26] كان `draftFromCategory` يرث `trackExpiry` من `expiry_batches`. العقد الحالي يجعل التتبع اختيارياً لكل صنف ويبدأ `trackExpiry: false` حتى لو فعّل القسم `expiry_batches`؛ لا تُفتح حقول الدفعات إلا بعد opt-in صريح للصنف.
- ثُبّتت سياسة الضريبة في POS والشراء على نسبة البلد مع احترام `resolveBusinessTax`: الإعفاء/عدم التسجيل يعيدان الصفر، وكل سطر يحتفظ بنسبة ضريبته الفعلية. حدّثت فحوص الضرائب الساكنة لتعكس مسار السياسة الحقيقي بدلاً من طلب نص قديم.
- أضيفت هوية المظهر/تخطيط POS للأنشطة الثمانية الجديدة (`trading`, `manufacturing`, `services`, `stationery`, `herbalist`, `building_materials`, `household`, مع الخريطة العامة)، مع تطبيق تخطيطات القائمة السريعة/الشبكة/البطاقات في شاشة الكاشير، وإكمال لوحة مواد البناء بلون معتمد.
- أضيف زر طباعة A5 مباشر لكل فاتورة في `SalesInvoicesPage`؛ يمر عبر `printModelWithTemplate` الموحد، مع بقاء قالب الإعداد الدائم مستقلاً وعدم تغيير الافتراضي الحراري.
- حدّثت Fixtures وفحوص الإنتاج لتنشئ مخزناً رئيسياً صالحاً وتمرر `outputWarehouseId`، وفحوص حذف الأصناف لتمرر اعتماد المالك، وفحوص تعديل الفواتير لتمرر سبباً صريحاً قبل اختبار الحواجز اللاحقة. أضيفت whitelist موثقة لفحص المصفوفات في `postSale`، واستُبعدت مقارنات الأنواع وخصائص DOM من فحص القيم التجارية.
- عُدّلت توقعات فحوص القوالب من 28 إلى 29 نشاطاً، وأُصلحت خريطة الألوان والفحوص العابرة لكل الأنشطة؛ كما يوثق اختبار `item_expiry_opt_in` أن تفعيل الصلاحية opt-in للصنف يعمل، وأن القسم لا يفرضها افتراضياً.
- النتيجة النهائية: حلقة `scripts/verify_*.mjs` كاملة = **159/159 سكربتاً ناجحاً، 0 فشل**. `npm run typecheck` ناجح، `npm run build` ناجح (تحذير حجم bundle فقط)، `npm run test:e2e` = **69 ملفاً / 282 اختباراً ناجحاً**، `npm run lint:src` = **0 أخطاء / 36 تحذيراً قائماً**، وفحص `node --check` لكل ملفات Electron ناجح. أمر `npm run test:e2e -- --runInBand` ليس عقداً صالحاً لـVitest؛ أُعيد تشغيل الأمر الصحيح دون الخيار.

### CONTINUATION-QUALITY-2026-09-25 — مراجعة ما بعد الإطلاق
- الحالة المرجعية الحالية هي شجرة الأنشطة الـ29، لا وصف المراحل التاريخي للـ16 أو الـ18 نشاطاً؛ العبارات التاريخية في هذا الملف تُقرأ كسجل زمني، بينما أرقام الجودة الحالية في هذا القسم هي المرجع التشغيلي.
- منتقيات الصنف/العميل/المورد/الموظف/المصروف كلها popover قابلة للكتابة والبحث من أول حرف والاختيار بـEnter؛ لا توجد عناصر HTML أصلية `<select` في `app/src`.
- عرض القائمة هو الافتراضي في شاشات الأطراف والأنشطة ذات تبويبات السجل/التقرير، ويحفظ التفضيل بمفتاح مستقل لكل قسم (`customers`, `suppliers`, `laundry`, `maintenance`, `rental-contracts`, `transfers`, `trips`) عبر `usePersistedSectionView` في التخزين المحلي؛ لا تُسجل تفضيلات العرض داخل دفتر الأعمال أو الحالة المحاسبية.
- لا توجد ملفات عمل غير مدفوعة بعد الدمج الأخير: الفرع نظيف، والـHEAD مدفوع إلى `origin/arena/01a0c540-shopsys` دون force push.

### VIEW-PREFERENCE-2026-09-25 — تعميم افتراضية القائمة وحفظ العرض
- أضيف `SectionViewPreference.ts` كطبقة واجهة صغيرة تقرأ/تكتب تفضيل العرض بمفتاح مستقل لكل قسم، مع fallback آمن إلى `list` عند غياب التخزين أو وجود قيمة غير معروفة.
- طُبق الحارس على شاشات المغسلة والصيانة والإيجارات والتحويلات والنقلات، إضافة إلى العملاء والموردين؛ اختيار التقرير/الأرصدة لا يضيع عند مغادرة الشاشة والعودة.
- التفضيل محلي واجهي فقط ولا يدخل في Zustand أو snapshot SQLite أو القيود المحاسبية. أضيفت 3 اختبارات لعزل الأقسام، القيمة الافتراضية، والقيم غير المعروفة.
- الفحص المستهدف: `npm run typecheck`، `npx vitest run tests/section_view_preference.test.ts`، و`oxlint` للملفات المتأثرة — كلها ناجحة.

### PARTY-PICKER-50 — منتقي الطرف الموحد في الفواتير (2026-09-25)
- وسّع `PartyQuickPicker` ليعرض الاسم، الكود الثابت (`CUS-####`/`SUP-####`)، الرصيد المحسوب من `getCustomerBalance` أو `getSupplierBalance`، والحالة؛ لا تظهر «موقوف» إلا عند `active === false`.
- البحث داخل المنتقي يقبل الاسم والهاتف والكود من أول حرف، ويظل اختيار «عميل نقدي»/«مورد نقدي» قيمة مباشرة بلا إنشاء طرف وهمي. Enter يؤكد القيمة وينقل التركيز إلى منتقي الأصناف عبر عقد `shopsys:focus-item`.
- رُبط السلوك بفاتورة البيع المتقدمة، فاتورة الشراء المتقدمة، فاتورة الشراء التقليدية، شاشة المبيعات القديمة، والكاشير، مع إبقاء منتقي الموظف متوافقاً عبر callback معلومات اختياري.
- أضيفت حالة `active?` الاختيارية للعميل للتوافق مع بيانات الأطراف المستقبلية، مع عدم افتراض إيقاف العميل عند غياب الحقل.
- الفحص المرحلي: `npm run typecheck` و`tests/keyboard_pickers.test.tsx` — 10 اختبارات ناجحة، مع اختبار عرض الكود/الرصيد/الحالة والبحث بالكود.

### PARTY-PICKER-50 — نتيجة الفحص النهائي
- بعد توحيد تسمية «مورد نقدي» في شاشة الشراء ومرتجع الشراء، نجحت `npm run typecheck` و`npm run build` (تحذير bundle الكبير غير حاجز).
- نجحت الاختبارات الكاملة: 70 ملفاً و286 اختباراً، ونجح `node --check desktop/main.cjs desktop/preload.cjs` و`git diff --check`.
- نجح `npm run lint:src` و`npm run lint` بصفر أخطاء؛ بقيت تحذيرات React/Compiler والتحذيرات القديمة في السكربتات كما هي دون إدخال تحذير جديد حاجز.

### UI-CONTRACT-51 — تصحيحات المالك الملزمة (2026-09-25)
- تتبع الصلاحية (`trackExpiry`/الدفعات) **اختياري بالكامل**: القيمة الافتراضية للصنف الجديد `false`، ولا يظهر تاريخ الصلاحية أو يطلب إدخاله إلا إذا فعّل العميل التتبع للصنف/النشاط. لا نضيف صلاحية افتراضية لأي فاتورة أو نشاط.
- العميل والمورد ليسا قائمة منسدلة: الحقل مربع كتابة حر، وعند بدء الكتابة فقط يفتح بحثاً منبثقاً حقيقياً بالاسم أو الهاتف أو الكود الفريد، وتعرض النتيجة الاسم والكود والرصيد، وتعرض «موقوف» فقط عند `active === false`.
- فاتورة البيع والشراء: المصروفات أساسية في كل الأنماط، وتظهر كزر صغير «إضافة مصاريف» يفتح نافذة عملية لا كقسم طويل دائم داخل الفاتورة. في أسفل الفاتورة يكون بوكس التحصيل/الدفع بجانب بوكس الإجمالي.
- النمط الافتراضي لكل فاتورة جديدة هو «مبسط» دائماً؛ لا يُحفظ نمط فاتورة سابقة كافتراضي للفواتير اللاحقة. تبقى أنماط البيع/الشراء المباشر والأنماط الاحترافية متاحة صراحةً، ولا تختفي المصروفات عند اختيار المبسط.
- F9 يجب أن ينفذ اعتماد/ترحيل فاتورة الشراء فعلياً حتى من داخل حقول النموذج، مع الحفاظ على الترحيل المحاسبي و`assertBalanced` والانتقال إلى `/purchases/invoices`.
- أي تغيير لاحق يمس هذه القواعد يجب أن يحدّث هذا الملف ووثيقة التصميم والاختبارات قبل اعتباره مكتملاً.

### UI-CONTRACT-52 — شكل الفاتورة، التحذير، وحماية التعديل (2026-09-25)
- محررات فواتير البيع والشراء واجهة تطبيق مكتبي صلبة وموحدة: حاوية واضحة، رأس ثابت، بطاقات متناسقة، وملخص الدفع/الإجمالي في أسفل الشاشة. لا نعيد أقساماً كبيرة للمصروفات داخل الصفحة؛ واجهة المصروفات شريط صغير وأزرار تفتح النوافذ المنبثقة.
- البيع بأقل من تكلفة الصنف تحذير إلزامي مرئي، والترحيل يطلب اعتماد المشرف؛ النواة تمنع الترحيل دون `priceFloorOverrideBy` حتى لو لم يضبط المستخدم حداً أدنى منفصلاً.
- بجوار العميل/المورد المختار زر تعديل سريع يفتح كل البيانات الأساسية والموسعة دون مغادرة الفاتورة، مع حفظ/تراجع وحارس تعديلات غير محفوظة.
- عمليات الفاتورة والتعديل لا تُغلق بالخطأ: الحارس ينبه قبل إغلاق المتصفح أو الضغط على الإلغاء/العودة، ويوفر «تراجع عن العملية» لفقد التغييرات عمداً و«متابعة التعديل» للبقاء.
- حارس التنقل الداخلي المسجل في `ui.tsx` تتعامل معه روابط Sidebar والرأس واختصار F3؛ لا تستخدم `navigate` جديداً لفعل تنقّل من محرر غير محفوظ دون تمريره عبر الحارس أو عبر `requestClose` الخاص بالصفحة.
- POS والمرتجعات تستخدمان منتقي الطرف نفسه وزر التعديل السريع عند وجود طرف فعلي، ولا يُسمح بإغلاق معالج مرتجع متغير دون قرار صريح.

### INVOICE-SEARCH-TOTALS-2026-09-25 — بحث الأطراف/الأصناف وتجميع الإجماليات
- `PartyQuickPicker` و`ItemQuickPicker` لا يفتحان النتائج عند التركيز: يفتحان فقط بعد بدء الكتابة، أو باختصار البحث الصريح. الطرف النقدي (`عميل نقدي`/`مورد نقدي`) قيمة مباشرة ولا يُنشأ له سجل وهمي.
- النقرة الواحدة على الطرف أو الصنف تحدد الصف فقط؛ الاعتماد يكون بالنقر المزدوج أو بالأسهم ثم Enter. تعرض نافذة الطرف الاسم والهاتف والكود والرصيد، وتعرض «موقوف» فقط عند `active === false`. تعرض نافذة الصنف الكود قبل الاسم وشريط تعديل/حركة/أسعار للصنف المحدد.
- إجراءات الصنف من داخل محرري الفواتير تمر عبر `guardNavigation` قبل الانتقال إلى `/inventory/items?edit=`, `/inventory/items?card=` أو `/sales/price-lists?item=` حتى لا تضيع الكتابة غير المحفوظة.
- `KeyboardNavigation` يعامل الأسهم داخل `tr[data-entry-row]` كتنقل مكاني بين الحقول فقط، ويمنع سلوك spinner للحقول الرقمية؛ عناصر البحث التي تحمل `data-enter-native` تحتفظ بأسهمها المحلية.
- محررا البيع والشراء يضعان الرأس/البحث والجدول والإجمالي/التحصيل داخل `invoice-shell` واحد مع رأس أزرق، جدول أبيض، ومنطقة إجماليات رمادية، مع `overflow-visible` خارج حاوية تمرير الجدول حتى لا تُقص النوافذ.
- ترتيب ملخص الفاتورة: إجمالي السطور، الخصم بالنسبة أو القيمة اليدوية (والصفر صالح)، الصافي، مصاريف العميل/المورد المحملة، الإجمالي المحمل، ثم المحصل وطريقته والمتبقي. مصروفات المحل الداخلية تبقى خارج الإجمالي المحمل وتظل ضمن payload والترحيل والربحية.
- الخصم اليدوي يُحوّل إلى نسبة حسابية قبل `computeTotals` في البيع، وفي الشراء يُوزّع على أسعار السطور الصافية قبل `postPurchase` حتى يؤثر في التكلفة والمستحق والضريبة والقيد، مع حفظه واستعادته من المسودة.
- جدول الأسعار يعرض الكود ثم الاسم والتكلفة ثم القطاعي، ويظل تحرير السعر القطاعي متاحاً حتى دون فئات خصم.

### COMPLETION-2026-09-26 — إغلاق فحوص F9 والاستحقاق
- عُدّلت الفحوص الخمسة القديمة التي كانت تبيع تحت التكلفة بلا اعتماد أو تتوقع وراثة الصلاحية؛ أصبحت حلقة `scripts/verify_*.mjs` **159/159 ناجحة** دون تخفيف `PriceFloorError`.
- أضيف إلى `postExchange` اعتماد `priceFloorOverrideBy`، ويمرر اعتماد المشرف الموجود في الاستبدال إلى البيع الجديد؛ بذلك لا يستطيع الاستبدال تجاوز حارس التكلفة/الحد الأدنى بلا اعتماد.
- العقد الحالي: `DATA_VERSION = 25`؛ إثبات `paidBy: 'payable'` يثبت التكلفة والاستحقاق فقط، والسداد عبر `settlePurchaseExpensePayable` ينشئ `Voucher` دفع `PV-####` مرتبطاً بـ`purchaseExpensePayableId` وقيداً متوازناً في اليومية.
- الفحوص المنفذة في هذه الجولة: Vitest **70 ملفاً / 293 اختباراً**، `typecheck` ناجح، `build` ناجح مع تحذير حجم bundle فقط، `lint` ناجح **0 أخطاء / 140 تحذيراً**، و`git diff --check` ناجح. لا توجد حزمة Electron أو سكربت فحص Electron في `app/package.json` الحالي.

### INVOICE-LINES-UI-54 — توحيد جدول فواتير البيع والشراء (2026-09-26)
- أضيف `app/src/ui/components/InvoiceLinesTable.tsx` كمكوّن عرض مشترك لجدول البنود في الفاتورة المتقدمة للبيع والشراء. يوحّد رأس القسم وعدّاد البنود ومنتقي الصنف، الحاوية، التمرير الأفقي، الترويسة، الحواف، الخلايا، التحذيرات، الحالة الفارغة، حقول الكمية/السعر وزر حذف الصف.
- بقيت أعمدة العملية الخاصة منفصلة داخل نفس الإطار: الخصم والتكلفة/الهامش للبيع، والمطلوب/المستلم/المرفوض وضريبة المدخلات وتاريخ الصلاحية الاختياري ونصيب المصروفات للشراء. الصلاحية لا تظهر إلا للصنف الذي فعّل `trackExpiry` صراحةً.
- أُعيد ترتيب ذيل فاتورة الشراء ليتطابق مع البيع بصرياً: بطاقة الدفع/المبلغ المدفوع بجانب ملخص الإجمالي والمستحق والمتبقي، مع إبقاء مصروفات الفترة والمصروفات المحملة والضريبة في الحسابات الأصلية دون تغيير منطق الترحيل.
- صُحح مفتاح نمط الإدخال في الشراء ليحفظ قيمة النمط (`simple`/`direct`/`profit`) لا النص العربي، وتم الحفاظ على منتقيات البحث المنبثقة واعتماد الصنف بالنقر المزدوج أو الأسهم ثم Enter فقط.
- التحقق بعد التغيير: `npm run typecheck` ناجح، `npm run build` ناجح (تحذير حجم bundle من Vite غير حاجز)، `npm run test:e2e -- --run` = 70 ملفاً / 293 اختباراً ناجحاً، `npm run lint` = 0 أخطاء مع تحذيرات قائمة، و`git diff --check` ناجح.

### DECIMAL-INPUT-55 — توحيد إدخال الكسور العشرية (2026-09-26)
- سبب العطل في جدول الأصناف أن الحقول كانت `type="number"` مربوطة بقيمة رقمية مخزنة مباشرة؛ عند كتابة `1.` كانت React تعيد الرسم بالقيمة المحولة فوراً، فتختفي النقطة وتتحول الضغطة التالية إلى رقم مختلف. كما كان `step` الافتراضي يفرض خطوة صحيحة في بعض المتصفحات.
- أضيف `DecimalInput` إلى `ui.tsx` ويحافظ على النص الوسيط أثناء الكتابة، بما في ذلك `1.` والأرقام العربية `١٫٥` والفاصلة `1,5`، ثم يسلّم القيمة الموحّدة للحقل الأب. استُخدم في المبالغ المحولة من minor units والمصروفات والخصومات والصلاحيات وغيرها.
- جدول البنود يستخدم مسودات نصية محلية لكل كمية/سعر/نسبة، لذلك لا تضيع العلامة العشرية أثناء التحديث الفوري، مع دعم الكمية الكسرية وسعر الوحدة ونسبة الخصم/الضريبة.
- أضيف `step="any"` و`inputMode="decimal"` إلى حقول الأرقام في الواجهات، ووسّع `toMinor`/`normalizeDigits` لقبول النقطة والفاصلة العربية والإنجليزية.
- أضيفت اختبارات `decimal_input.test.tsx` و`money_decimal.test.ts`؛ الاختبارات المستهدفة 5/5 ناجحة.

### INVOICE-REFERENCE-UI-56 — توحيد واجهة البيع والشراء (2026-09-26)
- أضيف غلاف POS تشغيلي موحد لمحرري البيع والشراء عبر `InvoicePOSFrame`: شريط علوي داكن، هوية المسودة، التاريخ والحالة والعملة والمستودع والطرف، وشريط اختصارات عملي لـF2/F5/F6/F8/F9 مع حالة اتصال واضحة. الأزرار تستدعي callbacks الصفحة الفعلية ولا تكرر منطق الترحيل.
- أعيد ترتيب جسم الفاتورة إلى عمود بنود كثيف وعمود مالي مكدس للدفع/التحصيل وملخص الإجمالي، مع بقاء المصروفات خلف أزرار «إضافة مصاريف» والنوافذ القائمة. يستخدم النوعان نفس CSS و`InvoiceLinesTable`، بينما تظل أعمدة الشراء/البيع الخاصة مشروطة بالنوع.
- لا يحتوي الكروم على بيانات أعمال ثابتة: اسم الطرف والمستودع والعملة والنمط مأخوذة من الـstore والحالة الحالية، أما رقم `NEW-SALE`/`NEW-PURCHASE` فهو معرف مرئي لمسودة لم تُرحّل بعد.
- حافظت إعادة التخطيط على `PartyQuickPicker` و`ItemQuickPicker` كنوافذ بحث فعلية: لا تفتح عند التركيز، تبدأ بالكتابة، النقرة تحدد فقط، والاعتماد بالنقر المزدوج أو الأسهم ثم Enter. كما بقيت حراسة المسودة، الإدخال العشري الوسيط، موافقة البيع تحت التكلفة، المصروفات، `assertBalanced`، وترحيل F9 دون تغيير.

### INVOICE-POS-UI-57 — إعادة بناء الغلاف المرئي بعد مراجعة المالك (2026-09-26)
- أزيلت طبقة العرض السابقة `InvoiceReferenceChrome` التي كانت تضيف شريطاً فوق الصفحة القديمة، واستبدلت بغلاف فعلي `InvoicePOSFrame` يملك شريط POS العلوي وشريط الأوامر وسياق المستند وورقة الفاتورة وشريط الإجراءات السفلي.
- لا يعاد استخدام رأس الفاتورة القديم أو تخطيطه كواجهة رئيسية؛ يبقى منه فقط محتوى الحقول والجدول والملخص والمنطق التشغيلي داخل الغلاف الجديد. لذلك يظهر التصميم الآن كمساحة POS مكتظة ذات ترويسة داكنة، سياق مستند مضغوط، جدول أبيض، عمود مالي، وشريط سفلي ثابت.
- أزرار الغلاف مرتبطة مباشرة بنفس callbacks الحالية للبحث والحفظ والاستعادة والطباعة والترحيل؛ لا توجد نسخة تجريبية أو بيانات ثابتة في الإجماليات أو الطرف أو المخزن.

### INVOICE-POS-UI-58 — رأس تحكم واحد ونوافذ بحث مرجعية (2026-09-26)
- أزيل تكرار أزرار الطباعة والحفظ والترحيل والاستعادة: مصدر التحكم الوحيد هو الشريط العلوي داخل `InvoicePOSFrame`، وأزيل شريط الأزرار السفلي، بينما بقي شريط الأوامر العلوي مقتصراً على بحث الطرف والصنف.
- نُقلت حقول النمط والطرف والمخزن والمرجع والتاريخ والضريبة من القسم القديم إلى `headerFields` داخل رأس POS نفسه. رأس الفاتورة هو الآن مكان التعديل الوحيد، ولا توجد نسخة ثانية أسفله لتغيير نفس البيانات.
- أعيد تصميم نوافذ `PartyQuickPicker` و`ItemQuickPicker` بصرياً كحوارات بحث كاملة: شريط عنوان داكن، بحث بارز، نتائج جدولية كثيفة، حالة تحديد واضحة، إجراءات تعديل/حركة/أسعار للصنف، وتذييل يشرح Enter والنقر المزدوج؛ لم يتغير عقد الاختيار أو الحماية من الاعتماد بنقرة واحدة.

### POS-REFERENCE-2026-09-26 — غلاف POS مستقل لمحرري الفواتير
- مسارا `/sales/invoices/new` و`/purchases/invoices/new` يعرضان `InvoicePOSFrame` كجذر مستقل بلا تكرار `Header` أو `Sidebar` العامين. الغلاف يحتوي شريط النظام، شريط تنقل POS، شريط اختصارات معلوماتي، وSidebar تشغيل خاصاً بالفاتورة.
- أزرار الرجوع/البحث/الاستعادة/الطباعة/حفظ المسودة/الترحيل موجودة في رأس POS فقط؛ شريط الاختصارات يعرض المفاتيح ولا يكرر أزرار التنفيذ، وإضافة الصنف الفعلية موجودة في شريط الإدخال السريع قبل الجدول.
- `InvoiceLinesTable` يدعم `showPicker={false}` عندما يحقن المحرر `ItemQuickPicker` في شريط الإدخال السريع، مع إبقاء القيمة الافتراضية متوافقة مع الشاشات الأخرى.
- بيانات النشاط والفرع والمستخدم والعملات والأطراف والأصناف والحسابات تمر من stores الصفحة؛ لا تُنسخ أرقام أو إجماليات العرض المرجعي إلى التطبيق. الترحيل، المصروفات، الحماية، ونوافذ البحث تبقى callbacks ومكونات قائمة.
- تحقق هذه الجولة: `npm run typecheck` و`npm run build` و`npm run test:e2e` و`git diff --check` ناجحة، و`npm run lint` بلا أخطاء مع تحذيرات الشجرة القائمة.

### POS-REAL-STATE-2026-09-26 — حالات حقيقية وتنقل لوحة المفاتيح
- لا تعرض واجهة POS اتصال الطابعة على أنه مؤكد: تعرض ما يستطيع المتصفح إثباته (`window.print`) مع إعداد القالب والطباعة التلقائية، وتوضح أن اتصال العتاد الفعلي لا يمكن إثباته من المتصفح. حالة الاتصال والمزامنة تعتمد على `navigator.onLine` و`useAppStore().sync` عبر `connectivityStatus`.
- زر «العودة إلى قسم المبيعات/المشتريات» ظاهر في Sidebar ورأس المستند، ويستخدم `onBack` مع حارس التعديلات غير المحفوظة.
- يبدأ محرر الفاتورة بتركيز العميل/المورد النقدي. Enter على الطرف ينقل التركيز إلى إدخال الصنف، والكتابة في حقل الطرف تفتح نافذة البحث. `QuickSelect` يعتمد الاختيار الأول بـEnter ثم يترك Enter التالي لحارس التنقل العام بدلاً من إعادة أول خيار.
- نوافذ البحث لها طبقة أعلى من حقل التشغيل، ولا يحمل حقل الطرف z-index أعلى من الـoverlay. حقل الصنف يحجز مساحة يمين فعلية للأيقونة حتى لا تغطي النص.
- جدول البنود يعرض رقم السطر والكود والصنف والمخزن/الكميات والسعر والخصم/الضريبة والتكلفة والهامش والإجمالي والإجراء، مع عداد وقيمة البنود الفعلية في شريط الجدول.

### SECURITY-QUALITY-P0-2026-09-26 — إغلاق بوابة الأمن والجودة
- قناة الدعم أصبحت بروتوكول v2: اعتماد جهاز 256-bit مخزن كبصمة فقط في KV، وتوقيع HMAC-SHA-256 لكل طلب مع timestamp وnonce لمنع replay. الواجهة تبني التوقيع من `METHOD\\nPATH\\nTIMESTAMP\\nNONCE\\nRAW_BODY`، والـWorker يرفض الطلب القديم أو المعاد.
- أزيل CORS wildcard من `cloud/worker.js`. يجب ضبط `SUPPORT_ALLOWED_ORIGINS` بقائمة origins الإنتاجية؛ origin غير موجود في القائمة يُرفض ولا تعاد له `Access-Control-Allow-Origin`.
- مفتاح تشفير مزامنة Supabase أصبح عشوائياً 256-bit بصيغة base64url بطول 43، منفصلاً عن اعتماد RLS. أضيف تدوير تشفيري يعيد تشفير اللقطة داخل الجهاز ويدفعها بمراجعة شرطية، مع ملف ربط مشفر للتعافي وتحديث بقية الأجهزة.
- `AppErrorBoundary` و`applog` لا يعرضان stack trace أو بيانات مالية؛ اللوج يزيل الاعتمادات وquery strings ومحارف التحكم، ومصائد الأخطاء لا تُثبت مرتين تحت StrictMode. أضيفت حدود المسؤولية القانونية والطبية في `docs/حدود_الامتثال_القانوني_والطبي.md`.
- CI صار بوابة إلزامية: `npm ci` ثم audit للإنتاج ولكامل الشجرة، build/typecheck، `lint:strict` بصفر تحذيرات، Vitest، ثم `verify:all` الذي يشغل كل بوابات `verify_*.mjs` ويفشل صراحة عند أي فشل.
- تحقق الإغلاق في 2026-09-26: `lint:strict` = 0 تحذير/خطأ، build ناجح (تحذير حجم bundle غير حاجز)، Vitest = 72 ملفاً و299 اختباراً ناجحاً، `verify:all` = 160/160، `verify:security` ناجح، و`npm audit` الإنتاجي والكامل = 0 vulnerabilities.
- قرار تشغيلي: لا تُحفظ أسرار الدعم أو التشفير أو RLS في السجلات؛ CORS يعتمد قائمة `SUPPORT_ALLOWED_ORIGINS` صريحة، وأي origin غير مسموح يُرفض فعلياً لا بمجرد حذف header.
