...
export function RecipesPage() {
  ...
  const exportOrders = () => {
    const headers=['الأمر','التاريخ','المنتج','مخزن التصنيع','رقم التشغيلة','تاريخ الصلاحية','الكمية','تكلفة الخامات','المصروفات','الإجمالي'];const values=productionOrders.map(order=>[order.orderNumber,order.date.slice(0,10),items.find(item=>item.id===order.productItemId)?.nameAr??'',warehouses.find(warehouse=>warehouse.id===order.warehouseId)?.nameAr??'',order.outputLotNumber??order.orderNumber,order.outputExpiryDate??'',order.producedQty,fmt(order.ingredientsCostMinor),fmt(order.overheadMinor),fmt(order.totalCostMinor)]);const esc=(v:unknown)=>`"${String(v??'').replaceAll('"','""')}"`;const csv='\ufeff'+[headers,...values].map(row=>row.map(esc).join(',')).join('\n');const url=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));const anchor=document.createElement('a');anchor.href=url;anchor.download='production-orders.csv';anchor.click();URL.revokeObjectURL(url)
  }
  const reset = () => {
  ...
  const submit = () => {
    try {
      ...
      const order = postProduction({ productItemId: productId, producedQty: output, ingredients: ingredientRows, expenses, warehouseId, date: productionDate, outputExpiryDate: outputExpiryDate || null, outputLotNumber: outputLotNumber || null, notes: [notes, varianceReason && `سبب فرق الكمية: ${varianceReason}`].filter(Boolean).join(' — ') })
  ...
  return <div className="space-y-3 pb-20" dir="rtl">
    <header className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-card-dark p-4">
      ...
      <div className="mt-3 grid md:grid-cols-[180px_180px_1fr_auto] gap-2 form-row"><Field label="تاريخ التصنيع"><input type="date" className={inputCls} value={productionDate} onChange={(e)=>setProductionDate(e.target.value)}/></Field><Field label="رقم تشغيلة الناتج" hint="يُولد من رقم الأمر إذا تُرك فارغاً"><input className={inputCls} value={outputLotNumber} onChange={(e)=>setOutputLotNumber(e.target.value)} placeholder="مثال LOT-2026-001"/></Field><Field label="تحميل تركيبة محفوظة"><select className={inputCls} value="" onChange={(e)=>{loadRecipe(Number(e.target.value));e.target.value=''}}><option value="">اختر تركيبة سابقة…</option>{recipes.filter(recipe=>recipe.mode==='prepped'&&recipe.isActive).map(recipe=><option key={recipe.id} value={recipe.id}>{items.find(item=>item.id===recipe.productItemId)?.nameAr} — ناتج {recipe.yieldQty}</option>)}</select></Field><Btn variant="ghost" onClick={saveAsRecipe} disabled={!productId||output<=0||!materials.some(row=>row.itemId)}>حفظ كتركيبة</Btn></div>
    </header>
