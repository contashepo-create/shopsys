...
- أحداث التدقيق الجديدة تُنسخ إلى جدول `audit_events` داخل معاملة حفظ اللقطة، مع إبقاء سجل Zustand للتوافق. هذا مرآة محلية وليس نقلاً نهائياً لمصدر الحقيقة.
- النسخة الثانية من المخطط تضيف مرآة `journal_entries` و`journal_lines` append-only، وتثبت القيود الجديدة مع اللقطة والتدقيق في معاملة واحدة.
