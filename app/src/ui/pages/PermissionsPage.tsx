...
import { Btn, DecimalInput, Field, inputCls, Modal, useToast, PinInput } from '../components/ui.tsx'
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      ...
      <Modal open={terminalFor != null} onClose={() => setTerminalFor(null)} title={`💳 ماكينات المستخدم — ${appUsers.find((u) => u.id === terminalFor)?.nameAr ?? ''}`} wide>
        {(() => {
          ...
          const labels = { charge: 'تحصيل', refund: 'رد', void: 'إلغاء', settle: 'تسوية', view_totals: 'عرض الإجماليات' }
          return <div className="space-y-3"><div className="flex justify-end gap-2"><Btn variant="ghost" onClick={() => save([])}>منع الكل</Btn><Btn onClick={() => save(paymentTerminals.map((terminal) => ({ terminalId: terminal.id, operations: ['charge', 'refund', 'void', 'settle', 'view_totals'] })), paymentTerminals[0]?.id)}>منح الكل</Btn></div>{paymentTerminals.map((terminal) => { const grant = access?.grants.find((row) => row.terminalId === terminal.id); return <div key={terminal.id} className="rounded-xl border p-3 space-y-2"><div className="flex gap-2"><b className="flex-1">{terminal.nameAr} · {terminal.code}</b><label className="text-xs"><input type="radio" aria-label="افتراضية" checked={access?.defaultTerminalId === terminal.id} disabled={!grant} onChange={() => save(access?.grants ?? [], terminal.id)}/> افتراضية</label></div><div className="grid grid-cols-2 sm:grid-cols-5 gap-1">{(['charge','refund','void','settle','view_totals'] as const).map((operation) => <label key={operation} className="text-[10px]"><input type="checkbox" checked={grant?.operations.includes(operation) ?? false} onChange={() => { const grants = access?.grants ?? []; const operations = grant?.operations ?? []; const nextOps = operations.includes(operation) ? operations.filter((item) => item !== operation) : [...operations, operation]; const next = [...grants.filter((row) => row.terminalId !== terminal.id), ...(nextOps.length ? [{ terminalId: terminal.id, operations: nextOps, maxAmountMinor: grant?.maxAmountMinor }] : [])]; save(next, access?.defaultTerminalId === terminal.id && !nextOps.length ? next[0]?.terminalId : (access?.defaultTerminalId ?? next[0]?.terminalId)) }} /> {labels[operation]}</label>)}</div>{grant && <label className="flex items-center gap-2 text-[10px]">حد العملية (0 = بلا حد)<DecimalInput min="0" value={grant.maxAmountMinor ? grant.maxAmountMinor / 10 ** currencyDecimals : ''} onValueChange={(value) => { const maxAmountMinor = value ? toMinor(value, currencyDecimals) : undefined; save((access?.grants ?? []).map((row) => row.terminalId === terminal.id ? { ...row, maxAmountMinor } : row), access?.defaultTerminalId) }} className={`${inputCls} !py-1 !w-32`}/></label>}</div>})}{!paymentTerminals.length && <div className="text-center text-slate-400">لا توجد ماكينات مسجلة</div>}<div className="text-left"><Btn onClick={() => setTerminalFor(null)}>تم</Btn></div></div>
        })()}
      </Modal>
                    ...
                    {grant && (
                      <label className="flex items-center gap-2 text-[10px] text-slate-500">
                        ...
                        <DecimalInput
                          ...
                          onValueChange={(value) => {
                            const maxAmountMinor = value ? toMinor(value, currencyDecimals) : null
