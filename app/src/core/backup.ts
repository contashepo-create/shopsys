...
export function parseBackup(text: string): BackupFile {
  ...
  try {
    ...
    throw new Error('الملف ليس JSON صالحاً — تأكد أنه ملف نسخة احتياطية من ShopSys')
  }
  if (obj?.format !== BACKUP_FORMAT) throw new Error('الملف ليس نسخة احتياطية من ShopSys')
