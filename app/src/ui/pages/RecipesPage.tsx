...
export function RecipesPage() {
  ...
  const openMaterialPicker = (rowId: string, search = '') => { setPickerRowId(rowId); setPickerSearch(search); }
  const addMaterial = () => setMaterials((rows) => [...rows, { id: crypto.randomUUID(), itemId: 0, code: '', query: '', qty: '', unitFactor: 1, unitName: '' }])
  ...
  return <div className="space-y-3 pb-20" dir="rtl">
      ...
      {tab === 'materials' ? <div className="p-4 space-y-3">
        <div className="grid grid-cols-[110px_1fr_130px_120px_110px_36px] gap-2 px-2 text-[10px] font-bold text-slate-400"><span>كود الصنف</span><span>اسم الخام / بحث</span><span>المتاح</span><span>الوحدة</span><span>الكمية</span><span/></div>
          ...
          return <div key={row.id} className="grid grid-cols-[110px_1fr_130px_120px_110px_36px] gap-2 items-center rounded-xl border border-slate-100 dark:border-slate-800 p-2">
            ...
            <input ref={(node) => { qtyRefs.current[row.id] = node }} className={inputCls} inputMode="decimal" value={row.qty} onChange={(event) => patchMaterial(row.id, { qty: event.target.value })} placeholder="0" />
    ...
    {pickerRowId && <div className="fixed inset-0 z-50 bg-slate-950/55 p-4 flex items-center justify-center" onMouseDown={() => setPickerRowId(null)}><div className="w-full max-w-2xl max-h-[75vh] overflow-hidden rounded-2xl bg-white dark:bg-card-dark shadow-2xl border" onMouseDown={(event) => event.stopPropagation()}><div className="p-4 border-b"><b>اختيار خامة للتصنيع</b><input autoFocus className={`${inputCls} mt-3`} value={pickerSearch} onChange={(event) => setPickerSearch(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); const first = activeItems.filter((candidate) => candidate.id !== productId && [candidate.nameAr, candidate.sku, ...(candidate.barcodes ?? [])].join(' ').toLowerCase().includes(pickerSearch.trim().toLowerCase()))[0]; if (first) chooseMaterial(pickerRowId, first.id) } }} placeholder="ابحث بالاسم أو الكود أو الباركود ثم Enter"/></div><div className="max-h-[52vh] overflow-auto p-2">{activeItems.filter((candidate) => candidate.id !== productId && [candidate.nameAr, candidate.sku, ...(candidate.barcodes ?? [])].join(' ').toLowerCase().includes(pickerSearch.trim().toLowerCase())).map((candidate) => <button key={candidate.id} onClick={() => chooseMaterial(pickerRowId, candidate.id)} className="w-full grid grid-cols-[110px_1fr_120px] gap-3 text-right p-3 rounded-xl hover:bg-amber-500/10 focus:bg-amber-500/10 focus:outline-none"><span className="font-mono text-xs">{candidate.sku || candidate.barcodes?.[0] || candidate.id}</span><b>{candidate.nameAr}</b><span className="text-xs text-slate-500">متاح {warehouseQty(candidate.id)}</span></button>)}</div></div></div>}
