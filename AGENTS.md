...
**قرار المالك المعتمد:** تثبيت المنصة والتحول التقني التدريجي يسبقان سد الفجوات الرأسية. لا تبنِ ميزات نشاط كبيرة جديدة فوق `localStorage` ثم تنقلها مرة أخرى. أثناء التحول تُنفذ فقط إصلاحات فقد البيانات/المحاسبة/التشغيل/القانون، وتحسينات صغيرة عالية الأثر لا تزيد اقتران التخزين. المسار الحرج: أمن وجودة ← فصل Repository ← SQLite وترحيل ← Electron وIPC وأجهزة ← تشغيل وامتثال ← تعميق الأنشطة. ممنوع تقديم غلاف Electron فوق localStorage على أنه نسخة Desktop نهائية.
...
- **المرحلة 0 — الدفعة 0.5 (2026-09-22):** أُلغيت الحاجة لاختراع أسرار يدوية: زرا توليد crypto 256-bit لسر AES واعتماد RLS، و`core/syncPairing.ts` يصدر/يستورد ملف `.tksync` مشفراً AES-GCM بكلمة مؤقتة 12+ لا تُحفظ، لنقل إعداد الربط كاملاً إلى جهاز جديد دون أسرار نصية. الاستيراد ذري بعد تحقق الإصدار/الشكل/GCM؛ كلمة خاطئة أو عبث يرفضان. اختبار `sync_pairing.test.ts` يغطي round-trip وإخفاء السر والرفض. لا ترسل الملف وكلمته في قناة واحدة. التالي: خطة تدوير اعتماد RLS دون قطع الأجهزة، ثم تحذيرات lint.
