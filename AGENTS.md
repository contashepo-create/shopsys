...
**الوضع الحالي: 159 سكربت تحقق ناجحاً، و69 ملف Vitest تضم 282 اختباراً ناجحاً، وصفر فشل.** أي ميزة جديدة = سكربت تحقق جديد يغطي القيود والرفض والتوازن.
...
- ورث `draftFromCategory` خاصية `trackExpiry` من `expiry_batches` مع إبقاء التجاوز الفردي ممكناً؛ لا يُفقد تتبع الدفعات عند إنشاء صنف من قسم أغذية/صيدلية/نشاط مشابه.
...
- عُدّلت توقعات فحوص القوالب من 28 إلى 29 نشاطاً، وأُصلحت خريطة الألوان والفحوص العابرة لكل الأنشطة؛ كما وثّق اختبار `item_expiry_opt_in` أن الوراثة الافتراضية صحيحة مع قابلية الإلغاء الفردي.
