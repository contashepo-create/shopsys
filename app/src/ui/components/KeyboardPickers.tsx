...
export function ItemQuickPicker({ items, onPick, placeholder = 'اكتب كود أو اسم الصنف ثم Enter', amountLabel, inputElementRef, listenForShortcut = true, onEdit, onMovement, onPrices }: PickerProps) {
  ...
  return <div ref={pickerRef} className="relative" data-enter-native="true">
    ...
    {open && <div className="fixed inset-0 z-[80] bg-slate-950/35 p-4 sm:p-8" onMouseDown={() => setOpen(false)}>
      <div role="dialog" aria-label="نتائج بحث الأصناف" className="mx-auto mt-[8vh] flex max-h-[78vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-brand-300/50 bg-white shadow-2xl dark:border-brand-700/50 dark:bg-card-dark" onMouseDown={(event) => event.stopPropagation()}>
      <div className="border-b border-slate-200 p-2 dark:border-slate-700"><div className="relative"><Search size={15} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-brand-500"/><input ref={(node) => { searchInputRef.current = node; setExternalRef(node) }} className={`${inputCls} pr-9`} value={query} placeholder={placeholder} aria-label="بحث الصنف" onChange={(event) => { setQuery(event.target.value); setIndex(0) }} onKeyDown={handleKeyDown}/></div></div>
...
export function PartyQuickPicker({ parties, value, onChange, cashLabel, label, onConfirm, autoFocus = false, cashValue, showCash = true, partyInfo }: { parties: Party[]; value: number; onChange: (id: number) => void; cashLabel: string; label: string; onConfirm?: () => void; autoFocus?: boolean; cashValue?: number; showCash?: boolean; partyInfo?: (party: Party) => PartyPickerInfo }) {
  ...
  const renderParty = (party: Party, row: number) => {
    ...
    return <button type="button" key={party.id} data-quick-option="true" data-value={String(party.id)} onMouseDown={(event) => event.preventDefault()} onClick={() => setIndex(row)} onDoubleClick={() => commit(party)} className={`w-full rounded-lg p-2 text-right ${row === index ? 'bg-brand-500/15 ring-1 ring-brand-500/30' : 'hover:bg-slate-500/10'}`}>
  ...
  return <div ref={pickerRef} className={`relative ${open ? 'z-[70]' : ''}`} data-enter-native="true" data-party-picker="true">
    ...
    {open && <div className="fixed inset-0 z-[60] bg-slate-950/35 p-4 sm:p-8" onMouseDown={() => setOpen(false)}>
      <div role="dialog" aria-label={`${label} — نتائج البحث`} className="mx-auto mt-[8vh] flex max-h-[78vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-brand-300/50 bg-white shadow-2xl dark:border-brand-700/50 dark:bg-card-dark" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 p-4 dark:border-slate-700"><div><b className="text-base">بحث {label.replace('بحث ', '')}</b><p className="text-xs text-slate-500">اكتب الاسم أو الهاتف أو الكود، ثم اختر بالأسهم أو الضغط مرتين</p></div><button type="button" aria-label="إغلاق البحث" className="rounded-lg px-2 py-1 text-slate-500 hover:bg-slate-500/10" onClick={() => setOpen(false)}>✕</button></div>
        <div className="border-b border-slate-200 p-3 dark:border-slate-700"><input ref={searchInputRef} className={inputCls} value={query} placeholder={`ابحث في ${label.replace('بحث ', '')}`} aria-label={label} autoComplete="off" onChange={(event) => { setQuery(event.target.value); setIndex(0) }} onKeyDown={handleKeyDown} /></div>
