...
export function TransfersPage() {
  const { transfers, warehouses, items, postTransfer, purchases, sales, saleReturns, purchaseReturns, productionOrders } = useDataStore()
  const toast = useToast()
  const whName = (id: number) => warehouses.find((w) => w.id === id)?.nameAr ?? '—'

  const stock = useMemo(() => computeWarehouseStock(items, warehouses, transfers, buildWarehouseDocs(purchases, sales, saleReturns, purchaseReturns, productionOrders)), [items, warehouses, transfers, purchases, sales, saleReturns, purchaseReturns, productionOrders])
