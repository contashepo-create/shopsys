...
export function ItemQuickPicker({ items, onPick, placeholder = 'اكتب كود أو اسم الصنف ثم Enter', amountLabel }: PickerProps) {
  ...
  return <div className="relative" data-enter-native="true">
...
export function PartyQuickPicker({ parties, value, onChange, cashLabel, label }: { parties: Party[]; value: number; onChange: (id: number) => void; cashLabel: string; label: string }) {
  const [query, setQuery] = useState('')
  const [open, setOpen] = useState(false)
  const [index, setIndex] = useState(0)
  const inputRef = useRef<HTMLInputElement>(null)
  ...
  useEffect(() => { const focus=()=>{inputRef.current?.focus();inputRef.current?.select();setOpen(true)};window.addEventListener('shopsys:focus-party',focus);return()=>window.removeEventListener('shopsys:focus-party',focus) },[])
  return <div className="relative" data-enter-native="true"><input ref={inputRef} className={inputCls} value={open?query:(selected?.nameAr??cashLabel)} aria-label={label} onFocus={()=>{setQuery('');setOpen(true)}} onChange={e=>{setQuery(e.target.value);setIndex(0);setOpen(true)}} onKeyDown={e=>{if(e.key==='ArrowDown'){e.preventDefault();setIndex(i=>Math.min(matches.length-1,i+1))}else if(e.key==='ArrowUp'){e.preventDefault();setIndex(i=>Math.max(0,i-1))}else if(e.key==='Enter'){e.preventDefault();const p=matches[index]??matches[0];if(p)onChange(p.id);else onChange(value);setOpen(false)}else if(e.key==='Escape'){e.preventDefault();setOpen(false)}}}/>{open&&<div className="absolute z-50 mt-1 w-full max-h-64 overflow-auto rounded-xl border bg-white dark:bg-card-dark shadow-2xl p-1"><button type="button" onMouseDown={e=>e.preventDefault()} onClick={()=>{onChange(value===-1?-1:0);setOpen(false)}} className="w-full p-2 text-right rounded-lg hover:bg-slate-500/10 font-bold">{cashLabel}</button>{matches.map((p,i)=><button type="button" key={p.id} onMouseDown={e=>e.preventDefault()} onClick={()=>{onChange(p.id);setOpen(false)}} className={`w-full flex justify-between p-2 rounded-lg ${i===index?'bg-brand-500/15':'hover:bg-slate-500/10'}`}><b>{p.nameAr}</b><span className="text-xs text-slate-500">{p.phone}</span></button>)}</div>}</div>
}
