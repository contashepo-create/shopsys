...
export function TelegramPage() {
  ...
  const testConnection = () => run('وصلت رسالة الاختبار', async () => {
    await callBot('sendMessage', { chat_id: telegram.chatId, text: `✅ ${setup.shopName || 'ShopSys'} متصل بالبوت بنجاح` })
  })
  ...
  const sendDaily = () => run('أُرسل تقرير اليوم', async () => {
    ...
    await callBot('sendMessage', {
      ...
      text: buildDailyReportText({
        shopName: setup.shopName || 'ShopSys',
        dateLabel: today,
  ...
  const sendLowStock = () => run('أُرسل تنبيه النواقص', async () => {
    ...
    await callBot('sendMessage', { chat_id: telegram.chatId, text: buildLowStockText(setup.shopName || 'ShopSys', alerts) })
  })
  ...
  const sendBackup = () => run('أُرسلت النسخة الاحتياطية', async () => {
    ...
    fd.set('caption', buildBackupCaption(setup.shopName || 'ShopSys', backup.createdAt))
