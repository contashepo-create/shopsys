...
- فحوص الإنهاء: `npm run typecheck` و`npm run build` ناجحان، `npm run lint:src` بصفر أخطاء و36 تحذيراً قائماً، و`npm run test:e2e` يمر بـ69 ملفاً و282 اختباراً. فحوص `scripts/verify_*.mjs` القديمة ما زالت تحتوي توقعات 28 نشاطاً/عقوداً تاريخية لا تطابق الشجرة الحالية؛ فُحُوص SQLite/IPC لا تزال ناجحة، وفحص المقاولات المتأثر بإلزام الطرف حُدّث ويمر بـ31/31.
