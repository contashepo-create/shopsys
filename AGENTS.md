...
- **المرحلة 0 — الدفعة 0.4 (2026-09-22):** أزيل SQL الواجهة الخطر `using(true)` وأضيف migration مراجع `supabase/migrations/202609220001_secure_store_rls.sql`: RLS مفروض، سياسات SELECT/INSERT/UPDATE معزولة ببصمة اعتماد متجر 256-bit، ولا DELETE للعميل. فصل `accessToken` عن سر AES، يرسل العميل X-Store-Id/X-Store-Access-Token، يخزن hash فقط، ويطالب صفوف السياسة القديمة مرة واحدة بنمط TOFU. سر التشفير صار 16 حرفاً كحد أدنى، والواجهة تولد اعتماداً سليماً. المرجع `docs/أمن_مزامنة_Supabase.md` والفحص `verify_sync_rls.mjs`. التالي: تقوية توليد/تدوير أسرار المزامنة وخطة الاسترداد، ثم تحذيرات lint.
