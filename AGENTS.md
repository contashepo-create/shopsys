...
- أضيف اختباران لجسر التخزين (قراءة/كتابة/حذف والتعارض)، وتحديث `desktop/README.md`. نقل جداول الأعمال من اللقطة إلى جداول SQLite التفصيلية ما زال مرحلة لاحقة.
- الفحص: `npm run typecheck`، اختبارات SQLite/التخزين/المحاكي — 7 اختبارات ناجحة، و`node --check` لملفات Electron.
