...
ok('تجزئة المالك محفوظة (لا الرقم)', S().ownerPinHash === ownerHash && !JSON.stringify(S().ownerPinHash).includes('1111'))
const empHash = await hashPin('2222')
...
await throwsAsync('دخول برقم خاطئ يُرفض', () => S().login(emp.id, '9999'), 'خاطئ')
await S().login(emp.id, '2222')
...
await throwsAsync('بعد استنفاد المحاولات: مقفول بالدقائق', () => S().login(null, '1111'), 'مقفول')
...
const newEmpHash = await hashPin('3333')
...
await throwsAsync('الرقم القديم لم يعد يعمل', () => S().login(emp.id, '2222'), 'خاطئ')
await S().login(emp.id, '3333')
