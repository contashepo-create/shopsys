...
- **لا يوجد مخزن مبهم في الفواتير**: أزل سياسة «مخزن غير محدد» من الواجهة. الكاشير يبدأ بالمخزن الافتراضي/الرئيسي. فاتورة الشراء إما مخزن واحد للفاتورة كلها أو خيار «تحديد المخزن لكل سطر» مع `warehouseId` محفوظ على `PurchaseLine`.
- **أرصدة المخازن تقرأ مخزن السطر أولاً**: `buildWarehouseDocs` يستخدم `line.warehouseId ?? invoice.warehouseId`، ومرتجعات الشراء المجمعة تُوزع على سطور الأصل بالترتيب حتى لا يخرج رصيد من مخزن لم يستلم.
...
- الفحوص الدائمة: `verify_owner_warehouse_shift_refinement.mjs` (11 محطة بنيوية) + `verify_owner_warehouse_shift_runtime.mjs` (تشغيلي: شراء مختلط المخازن + اعتماد إقفال وردية بفارق).
