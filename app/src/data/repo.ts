...
export interface Car {
  ...
  purchasePayment?: CarPaymentMode | null
  /** ما دُفع عند الشراء من خزينة/بنك */
  ...
  /** طريقة تحصيل البيع — mixed = قبض جزء والباقي على حساب العميل */
  salePayment?: CarPaymentMode
  /** ما حُصّل عند البيع من خزينة/بنك */
  salePaidMinor?: number
...
interface DataState {
  ...
  editSale: (args: {
    ...
    paidMinor: number
    treasury: TreasuryAccount
    /** تحصيلات متعددة تُحفظ كما هي عند التعديل (خزينة/ماكينة/موظف) */
    paymentAllocations?: { accountCode: string; amountMinor: number; note?: string; employeeId?: number }[]
    invoiceDiscountPercent: number
    customerReference?: string
    dueDate?: string
    ...
    (rawSet, get) => {
      ...
      postSale: (args) => {
        ...
        for (const expense of internalExpenses) {
          ...
          if (expense.custodyFileId != null) {
            if (expense.settlement !== 'paid_now') throw new Error(`مصروف «${expense.label}» المرتبط بعهدة يجب أن يكون مدفوعاً فوراً`)
            const file = state.custodyFiles.find((row) => row.id === expense.custodyFileId)
            if (!file) throw new Error(`ملف العهدة للمصروف «${expense.label}» غير موجود`)
            assertFileOpen(file)
            ...
            const remaining = summarizeCustody(state.custodyTxs.filter((row) => row.fileId === file.id)).remainingMinor
            if (payout > remaining) throw new Error(`المصروف «${expense.label}» أكبر من متبقي العهدة`)
          }
          if (expense.vehicleId != null && !state.vehicles.some((vehicle) => vehicle.id === expense.vehicleId)) throw new Error(`مركبة المصروف «${expense.label}» غير موجودة`)
        }
        for (const expense of internalExpenses.filter((row) => row.settlement === 'paid_now')) {
          if (expense.custodyFileId != null) continue
        ...
        const custodyExpenseTxs: CustodyTx[] = internalExpenses.filter((expense) => expense.custodyFileId != null && expense.amountMinor > 0).map((expense, index) => ({
          id: nextId(state.custodyTxs) + index, fileId: expense.custodyFileId!, type: 'expense', date: now.slice(0, 10),
          amountMinor: expense.taxTreatment === 'exclusive' ? expense.amountMinor + Math.round(expense.amountMinor * expense.taxPercent / 100) : expense.amountMinor,
          excessMinor: 0, description: `[sale:${saleId}] ${expense.label}`, treasury: null, projectId: expense.projectId ?? null, purchaseId: null, journalEntryId: entryId,
        }))
        set({
          ...
          ...(custodyExpenseTxs.length ? { custodyTxs: [...state.custodyTxs, ...custodyExpenseTxs] } : {}),
      ...
      postVoucher: (args) => {
        ...
        const entryId = nextId(state.journal)
        const now = new Date().toISOString()
        const transactionAt = args.date ? `${args.date}T${now.slice(11)}` : now
        ...
        const terminalTransaction = terminal && args.terminalPayment
          ? buildTerminalCharge({ terminal, documentType: 'receipt_voucher', documentId: voucherNumber, amountMinor: args.amountMinor, providerReference: args.terminalPayment.providerReference, occurredAt: transactionAt, userId: state.currentUserId ?? 0, cardLast4: args.terminalPayment.cardLast4 })
          : null
          ...
          for (const pat of linkedPatients) {
            ...
            if (take > 0) {
              clinicCollections = [...clinicCollections, {
                id: nextId(clinicCollections), patientId: pat.id, date: transactionAt,
        ...
        const vehicleCostEntries = args.kind === 'payment' && args.vehicleId != null
          ? [...state.vehicleCostEntries, {
              id: nextId(state.vehicleCostEntries), vehicleId: args.vehicleId, date: transactionAt.slice(0, 10), kind: 'cost' as const,
      ...
      editSale: (args) => {
        ...
        for (const expense of internalExpenses) {
          ...
          if (expense.custodyFileId != null) {
            ...
            assertFileOpen(file)
            const payout = expense.taxTreatment === 'exclusive' ? expense.amountMinor + Math.round(expense.amountMinor * expense.taxPercent / 100) : expense.amountMinor
            const previousSaleCustody = state.custodyTxs.filter((row) => row.fileId === file.id && row.description.includes(`[sale:${sale.id}]`)).reduce((sum, row) => sum + row.amountMinor - row.excessMinor, 0)
            const remaining = summarizeCustody(state.custodyTxs.filter((row) => row.fileId === file.id)).remainingMinor + previousSaleCustody
            if (payout > remaining) throw new Error(`المصروف «${expense.label}» أكبر من متبقي العهدة`)
          }
          if (expense.vehicleId != null && !state.vehicles.some((vehicle) => vehicle.id === expense.vehicleId)) throw new Error(`مركبة المصروف «${expense.label}» غير موجودة`)
        }
        ...
        const editCustodyExpenseTxs: CustodyTx[] = internalExpenses.filter((expense) => expense.custodyFileId != null && expense.amountMinor > 0).map((expense, index) => ({
          id: nextId(state.custodyTxs) + index, fileId: expense.custodyFileId!, type: 'expense', date: now.slice(0, 10),
          amountMinor: expense.taxTreatment === 'exclusive' ? expense.amountMinor + Math.round(expense.amountMinor * expense.taxPercent / 100) : expense.amountMinor,
          excessMinor: 0, description: `[sale:${sale.id}] ${expense.label}`, treasury: null, projectId: expense.projectId ?? null, purchaseId: null, journalEntryId: newEntryId,
        }))
        const keptCustodyTxs = state.custodyTxs.filter((row) => !row.description.includes(`[sale:${sale.id}]`))
        set({
          ...
          items: updatedItems,
          custodyTxs: [...keptCustodyTxs, ...editCustodyExpenseTxs],
        })
        return updatedSale
      },
      ...
      editPurchase: (args) => {
        ...
        if (!args.reason.trim()) throw new Error('سبب التعديل مطلوب لسجل التدقيق')
        const editedSupplierId = args.supplierId ?? inv.supplierId
        if (editedSupplierId !== 0 && !state.suppliers.some((supplier) => supplier.id === editedSupplierId && supplier.active !== false)) throw new Error('المورد المحدد غير موجود أو غير نشط')
        const supplierDocument = args.supplierInvoiceNumber?.trim()
        if (supplierDocument && state.purchases.some((row) => row.id !== inv.id && row.supplierId === editedSupplierId && row.supplierInvoiceNumber === supplierDocument)) throw new Error('رقم فاتورة المورد مسجل مسبقاً لهذا المورد')
      ...
      addProperty: (args) => {
        ...
        for (const unit of initialUnits) {
          const normalizedCode = unit.code.trim()
      ...
      getOpenSupplierInvoices: (supplierId) => {
        ...
        const carInvoices = state.cars
          .filter((car) => car.purchasePayment !== 'cash' && (car.purchaseDueMinor ?? 0) > 0 && car.supplierId === supplierId)
          .map((car) => {
            const entry = state.journal.find((journalEntry) => journalEntry.id === car.purchaseEntryId)
            ...
            return {
              ...
              date: entry?.date ?? '0000-00-00',
              dueMinor: car.purchaseDueMinor ?? car.purchaseCostMinor,
      ...
      addCar: (args) => {
        ...
        if (errors.length) throw new Error(errors.join(' — '))
        const paidMinor = args.paidMinor ?? (args.payment === 'cash' ? args.purchaseCostMinor : 0)
        if (!Number.isInteger(paidMinor) || paidMinor < 0 || paidMinor > args.purchaseCostMinor) throw new Error('المدفوع يجب أن يكون بين صفر وإجمالي السيارة')
        const dueMinor = args.purchaseCostMinor - paidMinor
        if (dueMinor > 0 && !args.supplierId) throw new Error('شراء السيارة الآجل يتطلب اختيار المورد — اختر مورداً مسجلاً أولاً')
        ...
        const lines = buildCarPurchaseEntry(args.purchaseCostMinor, args.payment, label, args.treasury ?? '1101', paidMinor)
        const now = new Date().toISOString()
        ...
        const car: Car = {
          ...
          purchasePaidMinor: paidMinor, purchaseDueMinor: dueMinor,
          supplierId: dueMinor > 0 ? args.supplierId ?? null : null, prepCostMinor: 0,
      ...
      addCarPrep: (carId, amountMinor, payment, description, treasury = '1101', paidMinor, payableBeneficiary = '') => {
        ...
        const entry: JournalEntry = {
          id: entryId, entryNumber: entryId, date: now.slice(0, 10),
          description: `تجهيز ${label}: ${description || '—'}${beneficiary ? ` — جهة الاستحقاق: ${beneficiary}` : ''}`,
      ...
      sellCar: (args) => {
        ...
        const paidMinor = args.paidMinor ?? (args.payment === 'cash' ? totals.totalMinor : 0)
        if (!Number.isInteger(paidMinor) || paidMinor < 0 || paidMinor > totals.totalMinor) throw new Error('المحصّل يجب أن يكون بين صفر وإجمالي البيع')
        const dueMinor = totals.totalMinor - paidMinor
        // ربط المشتري بسجل العملاء: أي جزء آجل يحتاج عميلاً مسجلاً يُتتبع بكشفه
        if (dueMinor > 0 && args.buyerCustomerId == null) {
        ...
        // حد الائتمان يطبق على الجزء المتبقي فقط، لا على المبلغ المحصل فوراً
        if (dueMinor > 0) guardCreditLimit(get(), args.buyerCustomerId, dueMinor, args.creditLimitOverrideBy)
        const label = `${car.make} ${car.model} ${car.year} (${car.plateOrVin})`
        const lines = buildCarSaleEntry(totals, args.payment, label, args.treasury ?? '1101', paidMinor)
        const now = new Date().toISOString()
        const entryId = nextId(state.journal)
        const entry: JournalEntry = {
          id: entryId, entryNumber: entryId, date: now.slice(0, 10),
        ...
        const updated: Car = {
          ...
          salePayment: dueMinor === 0 ? 'cash' : paidMinor === 0 ? 'credit' : 'mixed',
          salePaidMinor: paidMinor, saleTotalMinor: totals.totalMinor,
        }
        let terminalTransaction: PaymentTerminalTransaction | null = null
        if (args.terminalPayment) {
          if (paidMinor <= 0 || args.payment === 'credit') throw new Error('الماكينة تربط بالجزء المحصل الآن فقط')
          const terminal = state.paymentTerminals.find((row) => row.id === args.terminalPayment!.terminalId)
          if (!terminal || terminal.status !== 'active' || terminal.settlementAccountCode !== (args.treasury ?? '1101')) throw new Error('ماكينة الدفع أو حسابها غير صالح للتحصيل')
          if (state.paymentTerminalTransactions.some((row) => row.terminalId === terminal.id && row.providerReference === args.terminalPayment!.providerReference.trim() && row.kind === 'charge')) throw new Error('مرجع مزود الدفع مستخدم مسبقاً')
          const activeUser = state.appUsers.find((user) => user.id === state.currentUserId)
          if (activeUser && activeUser.roleId !== 'owner' && activeUser.paymentTerminalAccess) assertTerminalOperation(activeUser.paymentTerminalAccess, terminal.id, 'charge', paidMinor)
          terminalTransaction = buildTerminalCharge({ terminal, documentType: 'car_sale', documentId: car.id, amountMinor: paidMinor, providerReference: args.terminalPayment.providerReference, occurredAt: now, userId: state.currentUserId ?? 0, cardLast4: args.terminalPayment.cardLast4 })
        }
    ...
    }},
    {
      ...
      migrate: (persisted: unknown) => {
        ...
        return {
          ...
          // ترحيل شراء السيارات: السيارات القديمة لا تملك مورداً، والجديدة تحفظ طريقة الدفع والربط
          cars: (s.cars ?? []).map((car) => ({
            ...car,
            purchasePayment: car.purchasePayment ?? null,
            purchasePaidMinor: car.purchasePaidMinor ?? (car.purchasePayment === 'cash' ? car.purchaseCostMinor : 0),
            purchaseDueMinor: car.purchaseDueMinor ?? (car.purchasePayment === 'credit' ? car.purchaseCostMinor : 0),
