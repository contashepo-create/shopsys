...
└── app/                         ← تطبيق Vite + React 19 + TypeScript + Tailwind v4 + zustand v5
    ├── scripts/                 ← 52 سكربت تحقق verify_*.mjs + license_tool.mjs
...
# الحزمة الكاملة (52 سكربت — يجب 0 فشل):
for f in scripts/verify_*.mjs; do node --experimental-strip-types "$f" >/dev/null 2>&1 || echo "FAILED: $f"; done
```

**الوضع الحالي: 1735 اختباراً ناجحاً عبر 52 سكربت، صفر فشل.** أي ميزة جديدة = سكربت تحقق جديد يغطي القيود والرفض والتوازن.
...
| clothing ملابس | pos, inventory, purchases, installments | متغيرات لون/مقاس |
...
- ✅ منجز: كل الفجوات المعيارية عدا واحدة (وصفات، صاغة، قوائم أسعار، تكاليف معدات، أمانة، سائقون، تأمين) + عمق المقاولات الكامل.
- ⏳ **التالي المتفق عليه**: مصفوفة مخزون الملابس **لون×مقاس بأرصدة مستقلة لكل تركيبة** — تغيير بنيوي يمس الصنف والكاشير والشراء والجرد.
