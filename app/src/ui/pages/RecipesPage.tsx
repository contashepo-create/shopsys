...
export function RecipesPage() {
  ...
  return <div className="space-y-3 pb-20" dir="rtl">
    ...
    {productionOrders.length > 0 && <details className="rounded-xl border p-3"><summary className="cursor-pointer font-bold text-sm">آخر أوامر التصنيع ({productionOrders.length})</summary><div className="flex justify-end"><Btn variant="ghost" onClick={exportOrders}>تصدير Excel</Btn></div><div className="mt-2 space-y-1">{[...productionOrders].reverse().slice(0, 10).map((order) => <div key={order.id} className="grid md:grid-cols-7 gap-2 text-xs border-t py-2"><b>{order.orderNumber}</b><span>{items.find((item) => item.id === order.productItemId)?.nameAr}</span><span>{warehouses.find((warehouse) => warehouse.id === order.warehouseId)?.nameAr ?? '—'}</span><span>تشغيلة: {order.outputLotNumber ?? order.orderNumber}</span><span>صلاحية: {order.outputExpiryDate ?? '—'}</span><span>{order.producedQty}</span><span>{fmt(order.totalCostMinor)}</span></div>)}</div></details>}
  </div>
}
