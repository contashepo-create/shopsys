...
export function customerUnitDocs(args: {
  ...
  /** سيارات معرض بيعت آجلاً أو بمقدم لمشترٍ مربوط بهذا العميل */
  ...
  for (const c of args.cars ?? []) {
    if (c.buyerCustomerId !== args.customerId || c.salePayment === 'cash' || !c.soldAt) continue
    const total = c.saleTotalMinor ?? 0
    if (total <= 0) continue
    const paid = Math.min(total, Math.max(0, c.salePaidMinor ?? 0))
