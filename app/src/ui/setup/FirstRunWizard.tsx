/**
 * معالج أول تشغيل — 3 خطوات:
 * 1) اختيار البلد (يضبط العملة والكسور والضريبة تلقائياً)
 * 2) اختيار النشاط (يفعّل خصائص الأصناف ووحدات العمل)
 * 3) بيانات المحل وحساب المالك (كل الصلاحيات — محمي بنيوياً)
 */
import { useState } from 'react'
import { Check, ChevronLeft, Store, Crown, Sparkles } from 'lucide-react'
...
export function FirstRunWizard() {
  ...
  const canNext = step === 1 ? !!country : step === 2 ? !!activity : shopName.trim() && ownerName.trim()

  const finish = () => {
    if (country && activity && shopName.trim() && ownerName.trim()) {
      completeSetup({ country, activity, shopName: shopName.trim(), ownerName: ownerName.trim() })
    }
  }
    ...
    <div dir="rtl" className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-brand-600 via-violet-600 to-fuchsia-600 dark:from-slate-950 dark:via-brand-900 dark:to-fuchsia-950">
      <div className="w-full max-w-3xl anim-pop">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-3xl font-black text-white">أهلاً بك في ShopSys</h1>
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              ...
              {s < 3 && <div className={`w-10 h-1 rounded-full transition-colors duration-300 ${s < step ? 'bg-emerald-400' : 'bg-white/20'}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-card-dark rounded-3xl shadow-2xl p-6 md:p-8">
          ...
          <div className="flex items-center justify-between mt-6 pt-5 border-t border-slate-100 dark:border-slate-800">
            ...
            >
              {step === 3 ? '🚀 ابدأ العمل' : 'التالي'}
              {step < 3 && <ChevronLeft size={16} className="transition-transform duration-200 group-hover:-translate-x-1" />}
            </button>
          </div>
        </div>
      </div>
