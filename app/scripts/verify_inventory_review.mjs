...
console.log('\n— V1) حذف الأصناف بموانع السلامة —')
{
  ...
  st().removeItem(ghost.id, { approvedBy: 'المالك' })
  ...
  assert.throws(() => st().removeItem(traded.id, { approvedBy: 'المالك' }), /فواتير شراء/)
  ...
  assert.throws(() => st().removeItem(opening.id, { approvedBy: 'المالك' }), /رصيده/)
  ...
  st().removeItem(opening.id, { approvedBy: 'المالك' })
