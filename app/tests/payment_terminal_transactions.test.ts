...
import { remainingRefundableMinor, terminalDocumentKey, validateTerminalTransaction, type PaymentTerminalTransaction } from '../src/core/paymentTerminalTransactions.ts'
...
describe('عمليات ماكينة الدفع', () => {
 it('يفصل معرّف المستند نفسه حسب النشاط ويحافظ على توافق السجلات القديمة', () => {
  expect(terminalDocumentKey({ documentType: 'logistics_trip', documentId: '1' })).toBe('logistics_trip:1')
  expect(terminalDocumentKey({ documentType: 'project_extract', documentId: '1' })).toBe('project_extract:1')
